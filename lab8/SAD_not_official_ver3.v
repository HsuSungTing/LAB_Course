...
always@(*)begin
    case(cur_state)
        IDLE: begin
            if(in_valid_q==1'b1)next_state=INPUT;
        ...
        OUTPUT: begin
            if(mul_D_row_cnt==(T_qq-1) && mul_D_col_cnt==7) next_state=CLEAR;
...
//===step 0 take input===
reg cg_ctrl_Wk, cg_ctrl_Wq, cg_ctrl_Wv, cg_ctrl_X, cg_ctrl_Y;
wire cg_clk_Wk, cg_clk_Wq, cg_clk_Wv, cg_clk_X, cg_clk_Y;
GATED_OR GATED_Wk (
	.CLOCK(clk),
	.SLEEP_CTRL(cg_ctrl_Wk&&cg_en),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(cg_clk_Wk)
);
GATED_OR GATED_Wq (
	.CLOCK(clk),
	.SLEEP_CTRL(cg_ctrl_Wq&&cg_en),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(cg_clk_Wq)
);
GATED_OR GATED_Wv (
	.CLOCK(clk),
	.SLEEP_CTRL(cg_ctrl_Wv&&cg_en),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(cg_clk_Wv)
);
GATED_OR GATED_X (
	.CLOCK(clk),
	.SLEEP_CTRL(cg_ctrl_X&&cg_en),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(cg_clk_X)
);
GATED_OR GATED_Y (
	.CLOCK(clk),
	.SLEEP_CTRL(cg_ctrl_Y&&cg_en),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(cg_clk_Y)
);

always@(*)begin
    case(cur_state)
        EVAL_A, EVAL_scale,EVAL_relu, EVAL_SV:begin 
            cg_ctrl_Wk=1; cg_ctrl_Wq=1; cg_ctrl_Wv=1; 
            cg_ctrl_X=1; cg_ctrl_Y=1;
        end
        default: begin
            cg_ctrl_Wk=0; cg_ctrl_Wq=0; cg_ctrl_Wv=0; 
            cg_ctrl_X=0; cg_ctrl_Y=0;
        end
    endcase
end
...
//Part 3 Seq Logic
reg cg_en_q;
reg signed [5:0] in_data1_q;
reg [3:0] T_q;
reg signed [7:0] in_data2_q;
reg signed [7:0] w_Q_q, w_K_q, w_V_q;

always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        in_valid_q<=0;  cg_en_q<=0;
        in_data1_q<=0;  T_q<=0;
        in_data2_q<=0;
        w_Q_q<=0; w_K_q<=0; w_V_q<=0;
    end
    else begin
        in_valid_q<=in_valid;   cg_en_q<=cg_en;
        in_data1_q<=in_data1;   T_q<=T;
        in_data2_q<=in_data2;
        w_Q_q<=w_Q; w_K_q<=w_K; w_V_q<=w_V;
    end
end
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        T_qq<=1; w_cnt_stage<=0; w_in_row_cnt<=0; w_in_col_cnt<=0;   //input cnt
        x_in_row_cnt<=0; x_in_col_cnt<=0;
        y_in_row_cnt<=0; y_in_col_cnt<=0;
    end
    else if(in_valid_q==1'b1)begin
        //T_qq
        if(w_cnt_stage==0 && w_in_row_cnt==0 && w_in_col_cnt==0) T_qq<=T_q;
        //Wk Wq Wv
        ...
        // x
        if(x_in_row_cnt<T_qq && x_in_col_cnt==7)begin    //小cnt都數到底
        ...
        T_qq<=0; w_cnt_stage<=0; w_in_row_cnt<=0; w_in_col_cnt<=0;   //input cnt
        x_in_row_cnt<=0; x_in_col_cnt<=0;
        y_in_row_cnt<=0; y_in_col_cnt<=0;
    end
end
...
//Part 3 Seq Logic Wq
always@(posedge cg_clk_Wq or negedge rst_n)begin
    if(!rst_n)begin
...
//Part 3 Seq Logic Wk
always@(posedge cg_clk_Wk or negedge rst_n)begin
    if(!rst_n)begin
        for(i=0;i<8;i=i+1)begin
            for(j=0;j<8;j=j+1)begin
                Wk[i][j]<=8'd0; 
        ...
        if(w_cnt_stage==1) Wk[w_in_row_cnt][w_in_col_cnt]<=w_K_q; 
    end
    else if(cur_state==EVAL_A || cur_state==EVAL_scale || cur_state==EVAL_relu || cur_state== EVAL_SV)begin
        //shift reg array state
        for(i=0;i<8;i=i+1)begin
...
//Part 3 Seq Logic Wv
always@(posedge cg_clk_Wv or negedge rst_n)begin
    if(!rst_n)begin
        for(i=0;i<8;i=i+1)begin
            for(j=0;j<8;j=j+1)begin
                Wv[i][j]<=8'd0; 
        ...
        if(w_cnt_stage==2) Wv[w_in_row_cnt][w_in_col_cnt]<=w_V_q; 
    end
    else if(cur_state==EVAL_A || cur_state==EVAL_scale || cur_state==EVAL_relu || cur_state== EVAL_SV)begin
        //shift reg array state
        for(i=0;i<8;i=i+1)begin
...
//Part 3 Seq Logic X[]
always@(posedge cg_clk_X or negedge rst_n)begin
    if(!rst_n)begin
        for(i=0;i<8;i=i+1)begin
            for(j=0;j<8;j=j+1)begin
                X[i][j]<=8'd0;
        ...
        if(x_in_row_cnt<T_qq && x_in_col_cnt<=7)begin 
            X[x_in_row_cnt][x_in_col_cnt]<=in_data2_q;
        end
    end
    else if(cur_state==EVAL_A ||cur_state==EVAL_scale || cur_state==EVAL_relu || cur_state== EVAL_SV)begin
        for(i=0;i<8;i=i+1)begin
...
//Part 3 Seq Logic Y[]
always@(posedge cg_clk_Y or negedge rst_n)begin
    if(!rst_n)begin
        ...
        if(y_in_row_cnt<=3 && y_in_col_cnt<=3)begin
            Y[y_in_row_cnt][y_in_col_cnt]<=in_data1_q;
        end
    end
    else if(cur_state==EVAL_A ||cur_state==EVAL_scale || cur_state==EVAL_relu || cur_state== EVAL_SV)begin
        for(i=0;i<4;i=i+1)begin
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        ...
        for(i=0;i<8;i=i+1)begin
            for(j=0;j<7;j=j+1)begin
                K[i][j]<=K[i][j+1]; Q[i][j]<=Q[i][j+1]; 
            end
            K[i][7]<=K[i][0]; Q[i][7]<=Q[i][0]; 
        end
    end
...
wire signed [61:0] mul_out_step_2_5;
...
wire signed [12:0] part_1_det_out;      
...
reg signed [12:0] part_1_det_in_q;
wire signed [28:0] part_2_det_out;
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        ...
        if(mul_D_row_cnt<T_qq && mul_D_col_cnt==7)begin
...
module mul_comb (
    ...
    output signed [61:0] sum
);
    // products: each 59-bit signed
    wire signed [58:0] p0 = a0 * b0;
    wire signed [58:0] p1 = a1 * b1;
    wire signed [58:0] p2 = a2 * b2;
    wire signed [58:0] p3 = a3 * b3;
    wire signed [58:0] p4 = a4 * b4;
    wire signed [58:0] p5 = a5 * b5;
    wire signed [58:0] p6 = a6 * b6;
    wire signed [58:0] p7 = a7 * b7;
...
module det_eval_part1(
    ...
    input  signed [5:0] l,
    input  signed [5:0] o,
    output signed [12:0] part_1_det_out
);
    // x = (k * p) - (l * o)
...
module det_eval_part2(
    input  signed [6:0]  a,
    input  signed [6:0]  f,
    input  signed [12:0] part_1_det_in,
    output signed [28:0] part_2_det_out
);
