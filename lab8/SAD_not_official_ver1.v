...
//===step 0 take input===
reg cg_ctrl_input_arr;
wire cg_clk_input_arr;
GATED_OR GATED_input_arr (
	.CLOCK(clk),
	.SLEEP_CTRL(cg_ctrl_input_arr&&cg_en),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(cg_clk_input_arr)
);

always@(*)begin
    case(cur_state)
        EVAL_A, EVAL_scale,EVAL_relu, EVAL_SV: cg_ctrl_input_arr=1;
        default: cg_ctrl_input_arr=0;
    endcase
end
...
always@(posedge cg_clk_input_arr or negedge rst_n)begin
    if(!rst_n)begin
        ...
        for(i=0;i<8;i=i+1)begin
            for(j=0;j<7;j=j+1)begin
                Wk[i][j]<=Wk[i][j+1];   Wq[i][j]<=Wq[i][j+1]; 
                Wv[i][j]<=Wv[i][j+1];   X[i][j]<=X[i][j+1];
            end
            Wk[i][7]<=Wk[i][0];   Wq[i][7]<=Wq[i][0]; 
            Wv[i][7]<=Wv[i][0];   X[i][7]<=X[i][0]; 
        end
