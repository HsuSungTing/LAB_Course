...
always@(posedge clk or negedge rst_n)begin
    ...
    else if(cur_state==EVAL_A || cur_state==EVAL_scale || cur_state==EVAL_relu || cur_state== EVAL_SV)begin //shift
        for(i=0;i<8;i=i+1)begin
            for(j=0;j<7;j=j+1)begin
                Wq[i][j]<=Wq[i][j+1]; 
            end
...
always@(posedge clk or negedge rst_n)begin
    ...
    else if(cur_state==EVAL_scale || cur_state==EVAL_relu || cur_state==EVAL_SV)begin   //shift
        for(i=0;i<8;i=i+1)begin
            for(j=0;j<7;j=j+1)begin
    ...
    else if(cur_state==OUTPUT)begin     //shift
        for(i=0;i<8;i=i+1)begin
            for(j=0;j<7;j=j+1)begin
