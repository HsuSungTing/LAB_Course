...
//Part 3 Seq Logic Wq
reg cg_ctrl_Wq [0:7][0:7];
wire cg_clk_Wq [0:7][0:7];
genvar i_Wq,j_Wq;
generate
    for(i_Wq=0;i_Wq<8;i_Wq=i_Wq+1)begin
        for(j_Wq=0;j_Wq<8;j_Wq=j_Wq+1)begin
            GATED_OR GATED_Wq (
                .CLOCK(clk),.SLEEP_CTRL(cg_ctrl_Wq[i_Wq][j_Wq]&&cg_en),
                .RST_N(rst_n), .CLOCK_GATED(cg_clk_Wq[i_Wq][j_Wq])
            );
            always@(*)begin
                case(cur_state)
                    EVAL_A, EVAL_scale,EVAL_relu, EVAL_SV:cg_ctrl_Wq[i_Wq][j_Wq]=1; 
                    default: cg_ctrl_Wq[i_Wq][j_Wq]=0; 
                endcase
            end
            always@(posedge cg_clk_Wq[i_Wq][j_Wq] or negedge rst_n)begin
                if(!rst_n)begin
                    Wq[i_Wq][j_Wq]<=8'd0; 
                end
                else if(in_valid==1'b1)begin
                ...
                else if(cur_state==EVAL_A || cur_state==EVAL_scale || cur_state==EVAL_relu || cur_state== EVAL_SV)begin
                    if(j_Wq==7)Wq[i_Wq][7]<=Wq[i_Wq][0]; 
                    else  Wq[i_Wq][j_Wq]<=Wq[i_Wq][j_Wq+1]; 
                end
                else if(cur_state==CLEAR)begin
                    Wq[i_Wq][j_Wq]<=8'd0; 
                end
...
//Part 3 Seq Logic Wk
//Part 3 Seq Logic Wk
reg cg_ctrl_Wk [0:7][0:7];
wire cg_clk_Wk [0:7][0:7];
genvar i_Wk,j_Wk;
generate
    for(i_Wk=0;i_Wk<8;i_Wk=i_Wk+1)begin
        for(j_Wk=0;j_Wk<8;j_Wk=j_Wk+1)begin
            GATED_OR GATED_Wk (
                .CLOCK(clk),.SLEEP_CTRL(cg_ctrl_Wk[i_Wk][j_Wk]&&cg_en),
                .RST_N(rst_n), .CLOCK_GATED(cg_clk_Wk[i_Wk][j_Wk])
            );
            always@(*)begin
                case(cur_state)
                    EVAL_A, EVAL_scale,EVAL_relu, EVAL_SV, OUTPUT:cg_ctrl_Wk[i_Wk][j_Wk]=1; 
                    default: cg_ctrl_Wk[i_Wk][j_Wk]=0; 
                endcase
            end
            always@(posedge cg_clk_Wk[i_Wk][j_Wk] or negedge rst_n)begin
                if(!rst_n)begin
                    Wk[i_Wk][j_Wk]<=8'd0; 
                end
                else if(in_valid==1'b1)begin
                    ...
                    if(j_Wk==7)Wk[i_Wk][7]<=Wk[i_Wk][0]; 
                    else  Wk[i_Wk][j_Wk]<=Wk[i_Wk][j_Wk+1]; 
                end
                else if(cur_state==CLEAR)begin
                    Wk[i_Wk][j_Wk]<=8'd0; 
                end
...
//Part 3 Seq Logic Wv
reg cg_ctrl_Wv [0:7][0:7];
wire cg_clk_Wv [0:7][0:7];
genvar i_Wv,j_Wv;
generate
    for(i_Wv=0;i_Wv<8;i_Wv=i_Wv+1)begin
        for(j_Wv=0;j_Wv<8;j_Wv=j_Wv+1)begin
            GATED_OR GATED_Wv (
                .CLOCK(clk),.SLEEP_CTRL(cg_ctrl_Wv[i_Wv][j_Wv]&&cg_en),
                .RST_N(rst_n), .CLOCK_GATED(cg_clk_Wv[i_Wv][j_Wv])
            );
            always@(*)begin
                case(cur_state)
                    EVAL_A, EVAL_scale,EVAL_relu, EVAL_SV, OUTPUT:cg_ctrl_Wv[i_Wv][j_Wv]=1; 
                    default: cg_ctrl_Wv[i_Wv][j_Wv]=0; 
                endcase
            end
            always@(posedge cg_clk_Wv[i_Wv][j_Wv] or negedge rst_n)begin
                if(!rst_n)begin
                    Wv[i_Wv][j_Wv]<=8'd0; 
                end
                else if(in_valid==1'b1)begin
                    ...
                    if(j_Wv==7)Wv[i_Wv][7]<=Wv[i_Wv][0]; 
                    else  Wv[i_Wv][j_Wv]<=Wv[i_Wv][j_Wv+1]; 
                end
                else if(cur_state==CLEAR)begin
                    Wv[i_Wv][j_Wv]<=8'd0; 
                end
...
generate
    for(i_var=0;i_var<8;i_var=i_var+1)begin
        for(j_var=0;j_var<8;j_var=j_var+1)begin
            ...
            always@(*)begin
                case(cur_state)
                    EVAL_A, EVAL_scale,EVAL_relu,EVAL_SV, OUTPUT:cg_ctrl_X[i_var][j_var]=1; 
            ...
            always@(posedge cg_clk_X[i_var][j_var] or negedge rst_n)begin
                if(!rst_n)begin
                    ...
                    if(j_var==7) X[i_var][7]<=X[i_var][0]; 
                    else  X[i_var][j_var]<=X[i_var][j_var+1]; 
                end
                else if(cur_state==CLEAR)begin
                    X[i_var][j_var]<=8'd0;
                end
...
//Part 3 Seq Logic Y[]
reg cg_ctrl_Y [0:3][0:3];
wire cg_clk_Y [0:3][0:3];
genvar i_Y,j_Y;
...
generate
    for(i_Y=0;i_Y<4;i_Y=i_Y+1)begin
        for(j_Y=0;j_Y<4;j_Y=j_Y+1)begin
            GATED_OR GATED_Y (
                .CLOCK(clk),.SLEEP_CTRL(cg_ctrl_Y[i_Y][j_Y]&&cg_en),
                .RST_N(rst_n), .CLOCK_GATED(cg_clk_Y[i_Y][j_Y])
            );
            always@(*)begin
                case(cur_state)
                    EVAL_A, EVAL_scale,EVAL_relu,EVAL_SV, OUTPUT:cg_ctrl_Y[i_Y][j_Y]=1; 
                    default: cg_ctrl_Y[i_Y][j_Y]=0; 
                endcase
            end
            always@(posedge cg_clk_Y[i_Y][j_Y] or negedge rst_n)begin
                if(!rst_n)begin
                    Y[i_Y][j_Y]<=8'd0;
                end
                else if(in_valid==1'b1)begin
                    ...
                    if(j_Y==3) Y[i_Y][3]<=Y[i_Y][0]; 
                    else  Y[i_Y][j_Y]<=Y[i_Y][j_Y+1]; 
                end
                else if(cur_state==CLEAR)begin
                    Y[i_Y][j_Y]<=8'd0;
                end
...
reg cg_ctrl_K [0:7][0:7];
wire cg_clk_K [0:7][0:7];
genvar i_K,j_K;
generate
    for(i_K=0;i_K<8;i_K=i_K+1)begin
        for(j_K=0;j_K<8;j_K=j_K+1)begin
            GATED_OR GATED_K(
                .CLOCK(clk), .SLEEP_CTRL(cg_ctrl_K[i_K][j_K]&&cg_en),
                .RST_N(rst_n), .CLOCK_GATED(cg_clk_K[i_K][j_K])
            );
            always@(*)begin
                case(cur_state)
                    INPUT, EVAL_scale,EVAL_relu, EVAL_SV, OUTPUT:cg_ctrl_K[i_K][j_K]=1; 
                    default: cg_ctrl_K[i_K][j_K]=0; 
                endcase
            end
            always@(posedge cg_clk_K[i_K][j_K] or negedge rst_n)begin
                if(!rst_n)begin
                    K[i_K][j_K]<=16'd0; 
                end
                ...
                else if(cur_state==EVAL_scale || cur_state==EVAL_relu || cur_state==EVAL_SV)begin
                    if(j_K==7) K[i_K][7]<=K[i_K][0]; 
                    else K[i_K][j_K]<=K[i_K][j_K+1]; 
                end
                else if(cur_state==CLEAR)begin 
                    K[i_K][j_K]<=16'd0; 
                end
...
reg cg_ctrl_Q [0:7][0:7];
wire cg_clk_Q [0:7][0:7];
genvar i_Q,j_Q;
generate
    for(i_Q=0;i_Q<8;i_Q=i_Q+1)begin
        for(j_Q=0;j_Q<8;j_Q=j_Q+1)begin
            GATED_OR GATED_Q(
                .CLOCK(clk), .SLEEP_CTRL(cg_ctrl_Q[i_Q][j_Q]&&cg_en),
                .RST_N(rst_n), .CLOCK_GATED(cg_clk_Q[i_Q][j_Q])
            );

            always@(*)begin
                case(cur_state)
                    INPUT, EVAL_scale,EVAL_relu, EVAL_SV, OUTPUT:cg_ctrl_Q[i_Q][j_Q]=1; 
                    default: cg_ctrl_Q[i_Q][j_Q]=0; 
                endcase
            end
            always@(posedge cg_clk_Q[i_Q][j_Q] or negedge rst_n)begin
                if(!rst_n)begin
                    Q[i_Q][j_Q]<=16'd0; 
                end
                else if(cur_state==EVAL_KQV)begin       //EQAL
                    if(xw_big_cnt<=7 && xw_small_cnt<=7)begin
                    ...
                    if(j_Q==7) Q[i_Q][7]<=Q[i_Q][0]; 
                    else Q[i_Q][j_Q]<=Q[i_Q][j_Q+1]; 
                end
                else if(cur_state==CLEAR)begin 
                    Q[i_Q][j_Q]<=16'd0; 
                end
...
reg cg_ctrl_V [0:7][0:7];
wire cg_clk_V [0:7][0:7];
genvar i_V,j_V;
generate
    for(i_V=0;i_V<8;i_V=i_V+1)begin
        for(j_V=0;j_V<8;j_V=j_V+1)begin
            GATED_OR GATED_V(
                .CLOCK(clk), .SLEEP_CTRL(cg_ctrl_V[i_V][j_V]&&cg_en),
                .RST_N(rst_n), .CLOCK_GATED(cg_clk_V[i_V][j_V])
            );

            always@(*)begin
                case(cur_state)
                    INPUT, OUTPUT: cg_ctrl_V[i_V][j_V]=1; 
                    default: cg_ctrl_V[i_V][j_V]=0; 
                endcase
            end
            always@(posedge cg_clk_V[i_V][j_V] or negedge rst_n)begin
                if(!rst_n)begin
                    V[i_V][j_V]<=16'd0; 
                end
                else if(cur_state==EVAL_KQV)begin       //EVAL
                    ...
                    if(j_V==7) V[i_V][7]<=V[i_V][0]; 
                    else V[i_V][j_V]<=V[i_V][j_V+1]; 
                end
                else if(cur_state==CLEAR)begin 
                    V[i_V][j_V]<=16'd0; 
                end
...
//Part 3 Seq Logic AS[] (INPUT和EVAL_KQV, OUTPUT可關, 但要測試)
reg cg_ctrl_AS [0:7][0:7];
wire cg_clk_AS [0:7][0:7];
genvar i_AS,j_AS;
generate
    for(i_AS=0;i_AS<8;i_AS=i_AS+1)begin
        for(j_AS=0;j_AS<8;j_AS=j_AS+1)begin
            GATED_OR GATED_AS(
                .CLOCK(clk), .SLEEP_CTRL(cg_ctrl_AS[i_AS][j_AS]&&cg_en),
                .RST_N(rst_n), .CLOCK_GATED(cg_clk_AS[i_AS][j_AS])
            );

            always@(*)begin
                case(cur_state)     //不確定
                    INPUT, EVAL_KQV, OUTPUT: cg_ctrl_AS[i_AS][j_AS]=1; 
                    default: cg_ctrl_AS[i_AS][j_AS]=0; 
                endcase
            end
            always@(posedge cg_clk_AS[i_AS][j_AS] or negedge rst_n)begin
                if(!rst_n)begin
                    AS[i_AS][j_AS]<=0; 
                end
                else if(cur_state==EVAL_A)begin
                    ...
                    if(j_AS==7) AS[i_AS][7]<=AS[i_AS][0]; 
                    else AS[i_AS][j_AS]<=AS[i_AS][j_AS+1]; 
                end
                else if(cur_state==CLEAR)begin
                    AS[i_AS][j_AS]<=0; 
                end
...
//(INPUT和EVAL_KQV,EVAL_A,EVAL_scale,EVAL_relu 可關, 但要測試)
reg cg_ctrl_P [0:7][0:7];
wire cg_clk_P [0:7][0:7];
genvar i_P,j_P;
generate
    for(i_P=0;i_P<8;i_P=i_P+1)begin
        for(j_P=0;j_P<8;j_P=j_P+1)begin
            GATED_OR GATED_P(
                .CLOCK(clk), .SLEEP_CTRL(cg_ctrl_P[i_P][j_P]&&cg_en),
                .RST_N(rst_n), .CLOCK_GATED(cg_clk_P[i_P][j_P])
            );
            always@(*)begin
                case(cur_state)     //不確定
                    INPUT, EVAL_KQV, EVAL_A, EVAL_scale, EVAL_relu: cg_ctrl_P[i_P][j_P]=1; 
                    default: cg_ctrl_P[i_P][j_P]=0; 
                endcase
            end
            always@(posedge cg_clk_P[i_P][j_P] or negedge rst_n)begin
                if(!rst_n)begin
                    P[i_P][j_P]<=0;
                end
