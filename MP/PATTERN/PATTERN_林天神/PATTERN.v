...
parameter BEQ = 6;
parameter J   = 7;
parameter WAIT_MAX_WAIT_READY_CYCLE = `MAX_WAIT_READY_CYCLE;
...
initial begin
    ...
    setup_env_task;
    @(negedge clk);
    while(curr_inst_addr < 'h3000) begin
        wait_out_valid_task;
        check_ans_task;
...
task wait_out_valid_task; begin
    ...
    while(IO_stall === 1'b1) begin
        if(latency == WAIT_MAX_WAIT_READY_CYCLE) begin
            $display ("====================================================================");
            $display ("                                %0sFAIL!%0s                         ", txt_red_prefix, reset_color);
            $display ("            the execution latency is over %6d cycles                ", WAIT_MAX_WAIT_READY_CYCLE);
            $display ("====================================================================");  
            $finish;
        end
        latency = latency + 1;
