...
reg signed [15:0] golden_DRAM_data[0:4095];
// 
...
initial begin
    ...
    for( patcount=1 ; patcount<=5000 ; patcount=patcount+1 ) begin
        // 
        golden_pc = golden_pc + 2 ;
        if( golden_pc == 16'h3000 ) begin
            YOU_PASS_task;
            $finish;
        end
        ...
        if (golden_opcode===3'b000 && golden_func===1'b1)
            ...
            $display("Error: Wrong instruction format in PATTERN NO.%4d\t\tgolden_pc NO.%4x", patcount+1, golden_curr_pc);
...
task check_DRAM_data_task ; begin
    j = 0 ;
    for (i=16'h1000; i<16'h3000; i=i+2) begin  
        if (golden_DRAM_data[j]!=={ u_DRAM_data.DRAM_r[i+1] , u_DRAM_data.DRAM_r[i] }) begin
            ...
            $display ("      golden_DRAM_data[%4h(%4d)]: %4x(%8d) , your_DRAM_data[%4h(%4d)]: %4x(%8d)               ",
                     i, j, golden_DRAM_data[j], golden_DRAM_data[j],
                     i, j, { u_DRAM_data.DRAM_r[i+1] , u_DRAM_data.DRAM_r[i] },
                     { u_DRAM_data.DRAM_r[i+1] , u_DRAM_data.DRAM_r[i] });        
            $display ("--------------------------------------------------------------------------------------------");
            #(100);
        end
        j = j + 1 ;
