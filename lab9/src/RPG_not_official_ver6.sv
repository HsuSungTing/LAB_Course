...
always_comb begin
    case(cur_state)
        ...
        EVAL_login: begin
            if(EVAL_login_done==1) next_state = EVAL_dram_write;
            else next_state = EVAL_login;
        end
        EVAL_levelup: begin
            if(EVAL_levelup_done==1) next_state = EVAL_dram_write;
            else next_state = EVAL_levelup;
        end
        EVAL_battle: begin
            if(EVAL_battle_done==1) next_state = EVAL_dram_write;
            else next_state = EVAL_battle;
        end
        EVAL_skill: begin
            if(EVAL_skill_done==1) next_state = EVAL_dram_write;
            else next_state = EVAL_skill;
        end
        EVAL_inactive: begin
            if(EVAL_inactive_done==1) next_state = EVAL_dram_write;
...
logic [15:0] dmg_to_plyr, dmg_to_mons;
always_comb begin   //用comb logic
    if(mons_atk_q>=plyr_def_q) dmg_to_plyr=(mons_atk_q-plyr_def_q);
    else dmg_to_plyr=0;
    if(plyr_atk_q>=mons_def_q) dmg_to_mons=(plyr_atk_q-mons_def_q);
    else dmg_to_mons=0;
end
//step 2
logic [15:0] plyr_temp_HP_q, mons_temp_HP_q;
logic [15:0] plyr_temp_HP, mons_temp_HP;
always_comb begin
    if(dmg_to_plyr_q>=0) begin
        if(dmg_to_plyr_q>=plyr_HP_q) plyr_temp_HP=0;
        else plyr_temp_HP=plyr_HP_q-dmg_to_plyr_q;
    end
    else plyr_temp_HP=plyr_HP_q;

    if(dmg_to_mons_q>=0) begin
        if(dmg_to_mons_q>=mons_HP_q) mons_temp_HP=0;
        else mons_temp_HP=mons_HP_q-dmg_to_mons_q;
    end
    else mons_temp_HP=mons_HP_q;
end
//step 3
logic win_sat_bool, lose_sat_bool, tie_sat_bool;
logic [15:0] plyr_EXP_d, plyr_MP_d, plyr_HP_d, plyr_atk_d, plyr_def_d;
always_comb begin
    //win
    if(plyr_temp_HP_q>0 && mons_temp_HP_q<=0)begin  
        ...
        plyr_atk_d=plyr_atk_q;
        plyr_def_d=plyr_def_q;
    end
    //lose
    else if(plyr_temp_HP_q<=0)begin  
    ...
    //tie
    else if(plyr_temp_HP_q>0 && mons_temp_HP_q>0)begin
        plyr_HP_d=plyr_temp_HP_q;
        plyr_atk_d=plyr_atk_q;
        plyr_def_d=plyr_def_q;
...
always_ff@(posedge clk or negedge inf.rst_n) begin
    if(!inf.rst_n)begin
        ...
        //EVAL_levelup
        EVAL_levelup_cnt<=0; EVAL_levelup_warn<=0; EVAL_levelup_done<=0;
        Delta_MP_q<=0; Delta_HP_q<=0; Delta_ATK_q<=0; Delta_DEF_q<=0;
        //EVAL_battle
        EVAL_battle_cnt<=0; dmg_to_plyr_q<=0; dmg_to_mons_q<=0;
        ...
        EVAL_skill_cnt<=0;  EVAL_levelup_warn<=0;  EVAL_skill_warn<=0;
    end
    else if(cur_state==INPUT_and_dram_read)begin
        if(got_player_no==1)begin
            //case 2: R handshake done
        ...
        else if(EVAL_levelup_done==0&&EVAL_levelup_cnt==0)begin
            Delta_MP_q<=Delta_MP;   Delta_HP_q<=Delta_HP;
            Delta_ATK_q<=Delta_ATK; Delta_DEF_q<=Delta_DEF;
        end
        //case 2 計算更新後的attribute並更新
        else if(EVAL_levelup_done==0&&EVAL_levelup_cnt==1)begin
            plyr_MP_q<=Out_MP;      plyr_HP_q<=Out_HP;
            plyr_atk_q<=Out_ATK;    plyr_def_q<=Out_DEF;
            if(EVAL_satwarn_bool==1) EVAL_levelup_warn<=3'b101;
            else EVAL_levelup_warn<=EVAL_levelup_warn;
            EVAL_levelup_done<=1;
        end
    end
        ...
        if(EVAL_battle_cnt==0)begin     //用comb logic
            dmg_to_plyr_q<=dmg_to_plyr;  dmg_to_mons_q<=dmg_to_mons;
        end
        else if(EVAL_battle_cnt==1)begin
            plyr_temp_HP_q<=plyr_temp_HP; mons_temp_HP_q<=mons_temp_HP;
        end
        else if(EVAL_battle_cnt==2)begin            
            plyr_MP_q<=plyr_MP_d;   plyr_HP_q<=plyr_HP_d;
            plyr_atk_q<=plyr_atk_d; plyr_def_q<=plyr_HP_d;
        ...
        //step 1 find_ max skill num with min_mp
            ...
            EVAL_skill_done<=1;
            if(total_MP_consumed==0)EVAL_skill_warn<=3'b011;
            else plyr_MP_q<=plyr_MP_q-total_MP_consumed;
        end
    end
    else if(cur_state==CLEAR)begin
        //major signal
