...
task input_task ;  begin
    ...
    for (int i=0; i<3; i++) in_monster_val[i] = $urandom_range(0, 65535);
...
task cal_task; begin
    ...
    //===for debug===
    $display("GOLDEN PLAYER DATA: pat= %0d",i_pat);
    ...
    case(in_sel_action)
        Login: begin
            ...
            if (gold_cts_date) begin
                gold_plyr_EXP = gold_plyr_EXP + 16'd512;
                gold_plyr_MP  = gold_plyr_MP  + 16'd1024;
            end
            // Clamp 不能超過 65535
            if (gold_plyr_EXP > 16'd65535)begin
                gold_plyr_EXP = 16'd65535;
                gold_warn_msg=Saturation_Warn; gold_complete=0;
            end
            if (gold_plyr_MP > 16'd65535)begin
                gold_plyr_MP = 16'd65535;
                gold_warn_msg=Saturation_Warn; gold_complete=0;
            end
            // 回寫到 DRAM
        ...
        Battle: begin
            //in_monster_val[0] mons atk, in_monster_val[1] mons def, in_monster_val[2] mons HP
            if(gold_plyr_HP==0) begin
                gold_warn_msg=HP_Warn; gold_complete=0;
            end
            else begin
                //step 0
                gold_dmg_to_plyr = in_monster_val[0] - gold_plyr_def;
                gold_dmg_to_mons = gold_plyr_atk - in_monster_val[1];
                //step 1
                ...
                //lose
                else if(gold_plyr_HP_temp<=0 || gold_mons_HP_temp>0) begin
        ...
        Use_Skill: begin
            ...
            sum_all = int'(gold_sorted_mp[0]) + int'(gold_sorted_mp[1]) + int'(gold_sorted_mp[2]) + int'(gold_sorted_mp[3]);
            sum_3   = int'(gold_sorted_mp[0]) + int'(gold_sorted_mp[1]) + int'(gold_sorted_mp[2]);
            sum_2   = int'(gold_sorted_mp[0]) + int'(gold_sorted_mp[1]);
            sum_1   = int'(gold_sorted_mp[0]);
            // step 2: 根據 HP 扣除 MP
            if (sum_all < gold_plyr_HP) begin
                gold_plyr_MP = gold_plyr_MP - sum_all[15:0];
            end
            else if (sum_3 < gold_plyr_HP) begin
                gold_plyr_MP = gold_plyr_MP - sum_3[15:0];
            end
            else if (sum_2 < gold_plyr_HP) begin
                gold_plyr_MP = gold_plyr_MP - sum_2[15:0];
            end
            else if (sum_1 < gold_plyr_HP) begin
                gold_plyr_MP = gold_plyr_MP - sum_1[15:0];
            end
            else begin
...
begin
    ...
    if(in_training_type==0)begin
        //A
        gold_delta_MP=(gold_plyr_MP+gold_plyr_HP+gold_plyr_atk+gold_plyr_def)>>3;
        gold_delta_HP=(gold_plyr_MP+gold_plyr_HP+gold_plyr_atk+gold_plyr_def)>>3;
        gold_delta_atk=(gold_plyr_MP+gold_plyr_HP+gold_plyr_atk+gold_plyr_def)>>3;
        gold_delta_def=(gold_plyr_MP+gold_plyr_HP+gold_plyr_atk+gold_plyr_def)>>3;
    end
...
task check_cts_date_or_more_90DAYS(
    ...
    integer gold_plyr_mon_extend;
    integer cur_month_to_day;
    integer prev_month_to_day;
begin
    //====== 日期是否跨年 ======
    if ({gold_plyr_mon, gold_plyr_day}<{in_mon, in_day}) gold_plyr_mon_extend = gold_plyr_mon + 12;
    else gold_plyr_mon_extend = gold_plyr_mon;

    case (gold_plyr_mon_extend)
    ...
    endcase
    //====== 加上當月日期 ======
    cur_month_to_day  = cur_month_to_day  + gold_plyr_day;
    prev_month_to_day = prev_month_to_day + in_day;
    //====== 輸出 ======
