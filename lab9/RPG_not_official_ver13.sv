...
logic [2:0] EVAL_levelup_cnt;   //共2+3 cycle
logic [1:0] EVAL_battle_cnt;
logic [2:0] EVAL_skill_cnt;     //共2+2 cycle
...
always_comb begin
    if(plyr_temp_HP_q>0 && mons_temp_HP_q<=0)begin      //win 
        //sat_bool
        if(plyr_EXP_q>(65535-2048) || plyr_MP_q>(65535-2048)) win_sat_bool=1;
        else win_sat_bool=0;
        lose_sat_bool=0;
        //plyr
        if(plyr_EXP_q>(65535-2048)) plyr_EXP_d=65535; //sat
        else plyr_EXP_d=plyr_EXP_q+2048;
        if(plyr_MP_q>(65535-2048)) plyr_MP_d=65535;   //sat
    ...
    if(!inf.rst_n)begin
        ...
        if(EVAL_login_cnt==0)begin
            ...
            if(cts_bool_q==1)begin
                ...
                if(player_sig.Exp<=(65535-512) && player_sig.MP<=(65535-1204))begin
                ...
                //case 0-1 overflow 避免0verfloiw後更新
                    ...
                    if(player_sig.Exp>(65535-512))plyr_EXP_q<=65535;
                    else plyr_EXP_q<=plyr_EXP_q+512;

                    if(player_sig.MP>(65535-1024)) plyr_MP_q<=65535;
        ...
        //case 1-0 計算attribute的delta (因為sort, 故多了兩個cycle)
        ...
        //step 1 find_ max skill num with min_mp
            ...
            EVAL_skill_done<=1;
            if(can_use_skill_bool==0)EVAL_skill_warn<=MP_Warn;  //無法使用任何skill(故MP不變)
