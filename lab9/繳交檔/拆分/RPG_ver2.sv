...
always_ff@(posedge clk or negedge inf.rst_n) begin
    if(!inf.rst_n)begin
        ...
        //for sort
        for(j=0;j<4;j=j+1)begin
            s1_q[j]<=0; s2_q[j]<=0; s1c_q[j]<=0; s2c_q[j]<=0;
        end
    end
    else if(cur_state==EVAL_levelup)begin
        ...
        //case 1-0 find attribute delta (sort, so 2 more cycles)
            ...
            for(j=0;j<4;j=j+1)begin     //sort stage 1
                s1_q[j]<=s1[j];     s1c_q[j]<=s1c[j];
            end 
        end
        else if(EVAL_levelup_cnt==1 && EVAL_levelup_done==0)begin
            for(j=0;j<4;j=j+1)begin     //sort stage 2
                s2_q[j]<=s2[j];     s2c_q[j]<=s2c[j];
            end
        end
        ...
        if(EVAL_skill_cnt==0)begin          
            ...
            for(j=0;j<4;j=j+1) sorted_MP_q[j]<=sorted_Ai_d[j];
        end
