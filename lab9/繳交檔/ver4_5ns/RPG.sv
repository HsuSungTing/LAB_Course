...
logic [1:0] EVAL_login_cnt;
logic [2:0] EVAL_levelup_cnt;   //2+3 cycle
logic [1:0] EVAL_battle_cnt;
logic [2:0] EVAL_skill_cnt;     //2+2 cycle
logic [1:0] EVAL_inactive_cnt;
...
always_comb begin
    case(cur_state)
        ...
        EVAL_login: begin
            if(EVAL_login_cnt==1)next_state=EVAL_dram_write;
            else next_state=EVAL_login;
        end
        EVAL_levelup: begin
            if(EVAL_levelup_done==1 || EVAL_levelup_cnt==4) next_state=EVAL_dram_write;
        ...
        EVAL_skill: begin
            if(EVAL_skill_done==1 || EVAL_skill_cnt==3) next_state=EVAL_dram_write;
            else next_state = EVAL_skill;
        end
        EVAL_inactive: begin
            if(EVAL_inactive_cnt==1)next_state=EVAL_dram_write;
...
always_ff @(posedge clk or negedge inf.rst_n) begin
    if(!inf.rst_n)begin 
        ...
        date_mon_q<=inf.D.d_date[0].M; date_day_q<=inf.D.d_date[0].D; got_date<=1;
    end
    else if(cur_state==CLEAR)begin 
        date_mon_q<=0; date_day_q<=0; got_date<=0;
    end
...
//player_no input done then read dram, all valid, when valid=1 then take input
always_ff@(posedge clk or negedge inf.rst_n) begin
    if(!inf.rst_n)begin
        INPUT_done<=0; 
    end
...
logic [7:0] plyr_mon_q; logic [7:0] plyr_day_q;
...
always_ff@(posedge clk or negedge inf.rst_n) begin
    if(!inf.rst_n)begin
        ...
        //case 1: wait for AW handshake
        //case 2: W handshake done
        if((inf.W_VALID==1 && inf.W_READY==1) && AW_done==1 && W_done==0)begin
            inf.W_DATA<=0;
            W_done<=1;
            inf.W_VALID<=0;
        end
...
always_ff@(posedge clk or negedge inf.rst_n) begin
    if(!inf.rst_n)begin
        ...
        //case 3: wait for W handshake 
        //case 4: B handshake done
        if((inf.B_VALID==1 && inf.B_READY==1) && AW_done==1 && W_done==1 && dram_write_done==0)begin
            dram_write_done<=1;
            inf.B_READY<=0;
        end
        //case 5: wait for B handshake
...
//Part 2 Comb Logicdate check for 2 cts day and more than 90 days
logic cts_bool_d, more_than_90days_d;
logic cts_bool_q, more_than_90days_q;   //reg
check_date check_date_inst(
    .prev_mon(plyr_mon_q[3:0]),
    .prev_day(plyr_day_q[4:0]),
    .cur_mon(date_mon_q),
    .cur_day(date_day_q),
    .cts_bool(cts_bool_d),
    .more_than_90days(more_than_90days_d)
);
//Part 2 Comb Logic for EVAL_levelup
logic [15:0] find_delta_in [0:3];
logic [15:0] Delta_MP,Delta_HP,Delta_ATK,Delta_DEF;
logic [15:0] result_A_d;
logic [15:0] sorted_Ai_d [0:3];   //for type B & EVAL_skill
logic [1:0] sorted_char_d [0:3];  //for type B & EVAL_skill
logic [15:0] out_attr_arr_C_d [0:3];
logic [15:0] out_attr_arr_D_temp_d [0:3];

//only ot share sort module
always_comb begin
...
//find_delta Part 1
find_delta_part1 find_delta_part1_inst(
    .type_in(train_type_q),
    .MP(plyr_MP_q),  .HP(plyr_HP_q),
    .ATK(plyr_atk_q), .DEF(plyr_def_q),
    //type A output
    .result_A(result_A_d),  
    //type C output
    .out_attr_arr_C_0(out_attr_arr_C_d[0]), 
    .out_attr_arr_C_1(out_attr_arr_C_d[1]),
    .out_attr_arr_C_2(out_attr_arr_C_d[2]),
    .out_attr_arr_C_3(out_attr_arr_C_d[3]),
    //type D output
    .out_attr_arr_D_temp_0(out_attr_arr_D_temp_d[0]),  
    .out_attr_arr_D_temp_1(out_attr_arr_D_temp_d[1]),
    .out_attr_arr_D_temp_2(out_attr_arr_D_temp_d[2]),
    .out_attr_arr_D_temp_3(out_attr_arr_D_temp_d[3])
);

logic [15:0] result_A_q;
logic [15:0] sorted_Ai_q [0:3];   //for type B & EVAL_skill
logic [1:0] sorted_char_q [0:3];  //for type B & EVAL_skill
logic [15:0] out_attr_arr_C_q [0:3];
logic [15:0] out_attr_arr_D_temp_q [0:3];

//Part 2 Comb Logic for sort
// sort_stage1 <-> sort_stage2 
logic [15:0] s1[3:0], s2[3:0];
logic [1:0]  s1c[3:0], s2c[3:0];

logic [15:0] s1_q[3:0], s2_q[3:0];      //reg
logic [1:0]  s1c_q[3:0], s2c_q[3:0];    //reg

sort_stage1 S1(  //sort sort_stage1
    .in0(find_delta_in[0]), .in1(find_delta_in[1]), 
    .in2(find_delta_in[2]), .in3(find_delta_in[3]),
...
sort_stage2 S2(  //sort sort_stage2
    .s1_0(s1_q[0]), .s1_1(s1_q[1]), .s1_2(s1_q[2]), .s1_3(s1_q[3]),
    .s1_c0(s1c_q[0]), .s1_c1(s1c_q[1]), .s1_c2(s1c_q[2]), .s1_c3(s1c_q[3]),
    .s2_0(s2[0]), .s2_1(s2[1]), .s2_2(s2[2]), .s2_3(s2[3]),
    .s2_c0(s2c[0]), .s2_c1(s2c[1]), .s2_c2(s2c[2]), .s2_c3(s2c[3])
);

sort_stage3 S3(  //sort sort_stage3
    .s2_0(s2_q[0]), .s2_1(s2_q[1]), .s2_2(s2_q[2]), .s2_3(s2_q[3]),
    .s2_c0(s2c_q[0]), .s2_c1(s2c_q[1]), .s2_c2(s2c_q[2]), .s2_c3(s2c_q[3]),
    .out0(sorted_Ai_d[0]), .out1(sorted_Ai_d[1]), 
    .out2(sorted_Ai_d[2]), .out3(sorted_Ai_d[3]),
    .out_c0(sorted_char_d[0]), .out_c1(sorted_char_d[1]), 
    .out_c2(sorted_char_d[2]), .out_c3(sorted_char_d[3])
);

find_delta_part2 find_delta_part2_inst(
    .type_in(train_type_q),
    //type A (MP+HP+ATK+DEF)>>3;
    .result_A(result_A_q),      
    //type B input
    .sorted_A0(sorted_Ai_q[0]), 
    .sorted_A1(sorted_Ai_q[1]),
    .sorted_A2(sorted_Ai_q[2]),
    .sorted_A3(sorted_Ai_q[3]),
    .char_A0(sorted_char_q[0]),
    .char_A1(sorted_char_q[1]),
    .char_A2(sorted_char_q[2]),
    .char_A3(sorted_char_q[3]),
    //type C input
    .out_attr_arr_C_0(out_attr_arr_C_q[0]),
    .out_attr_arr_C_1(out_attr_arr_C_q[1]),
    .out_attr_arr_C_2(out_attr_arr_C_q[2]),
    .out_attr_arr_C_3(out_attr_arr_C_q[3]),
    //type D input
    .out_attr_arr_D_temp_0(out_attr_arr_D_temp_q[0]),  
    .out_attr_arr_D_temp_1(out_attr_arr_D_temp_q[1]),
    .out_attr_arr_D_temp_2(out_attr_arr_D_temp_q[2]),
    .out_attr_arr_D_temp_3(out_attr_arr_D_temp_q[3]),
    //output
    .Delta_MP(Delta_MP),    .Delta_HP(Delta_HP),       //output
...
logic can_use_skill_bool;
always_comb begin
    two_mp_sum=sorted_MP_q[0]+sorted_MP_q[1];
    three_mp_sum=two_mp_sum+sorted_MP_q[2];
    four_mp_sum=three_mp_sum+sorted_MP_q[3];

    if(four_mp_sum<=plyr_MP_q)begin 
        total_MP_consumed=four_mp_sum;  can_use_skill_bool=1;
    end
    else if(three_mp_sum<=plyr_MP_q)begin 
        total_MP_consumed=three_mp_sum; can_use_skill_bool=1;
    end
    else if(two_mp_sum<=plyr_MP_q)begin  
        total_MP_consumed=two_mp_sum;   can_use_skill_bool=1;
    end
    else if(sorted_MP_q[0]<=plyr_MP_q)begin 
        total_MP_consumed=sorted_MP_q[0]; can_use_skill_bool=1;
    end
    else begin //can't use any MP
        total_MP_consumed=0;  can_use_skill_bool=0; 
    end
end
...
always_ff@(posedge clk or negedge inf.rst_n) begin
    if(!inf.rst_n)begin
        ...
        //step 0 find cts_bool
        //step 1 update date & warn
        if(EVAL_login_cnt==1)begin
            //step 0  update date
            plyr_mon_q<=date_mon_q; plyr_day_q<=date_day_q;
            //step 1  update EXP or MP (if cts login)
            if(cts_bool_q==1)begin
                //case 0-0 no overflow add and update
                if(player_sig.Exp+512<=65535 && player_sig.MP+1024<=65535)begin
                    plyr_EXP_q<=plyr_EXP_q+512; plyr_MP_q<=plyr_MP_q+1024;
                end
                //case 0-1 overflow clamp then update
                else begin
        ...
        //case 0 check EXP warn
        //case 1-0 find attribute delta (sort, so 2 more cycles)
        //case 1-1 find attribute delta
        //case 1-2 update attribute
        if(EVAL_levelup_cnt==4 && EVAL_levelup_done==0)begin
        ...
        if(EVAL_battle_cnt==0)begin     //comb logic
            //step 0 check HP (if 0-> HP warn)
        ...
        //step 0 sort (sort, so 2 more cycles)
        //step 1 find_ max skill num with min_mp
        if(EVAL_skill_cnt==3)begin
            if(can_use_skill_bool==1)plyr_MP_q<=plyr_MP_q-total_MP_consumed;        //can use MP so update MP
        end
    end
    else if(cur_state==CLEAR)begin
        //major signal
...
//EVAL_login & EVAL_inactive
always_ff@(posedge clk or negedge inf.rst_n) begin
    if(!inf.rst_n)begin
        //EVAL_login 
        EVAL_login_cnt<=0; cts_bool_q<=0;
        EVAL_login_warn<=No_Warn; EVAL_login_done<=0;
        //EVAL_inactive
        EVAL_inactive_cnt<=0; more_than_90days_q<=0;
        EVAL_inactive_warn<=No_Warn; EVAL_inactive_done<=0;
    end
    else if(cur_state==EVAL_login)begin
        EVAL_login_cnt<=EVAL_login_cnt+1; 
        if(EVAL_login_cnt==0)begin
            cts_bool_q<=cts_bool_d;
        end
        else if(EVAL_login_cnt==1)begin
            //step 0  update date
            //step 1  update EXP or MP (if cts login)
            if(cts_bool_q==1)begin
                //case 0-0 no overflow add and update
                if(player_sig.Exp+512<=65535 && player_sig.MP+1024<=65535)EVAL_login_warn<=EVAL_login_warn;
                //case 0-1 overflow clamp then update
                else EVAL_login_warn<=Saturation_Warn;    //sat warn
            end
            EVAL_login_done<=1; //step 2 done
        end
    end
    else if(cur_state==EVAL_inactive)begin
        EVAL_inactive_cnt<=EVAL_inactive_cnt+1;
        if(EVAL_inactive_cnt==0)begin
            more_than_90days_q<=more_than_90days_d;
        end
        else if(EVAL_inactive_cnt==1)begin
            //step 1 check 90 days
            if(more_than_90days_q==1)EVAL_inactive_warn<=Date_Warn;
...
always_ff@(posedge clk or negedge inf.rst_n) begin
    if(!inf.rst_n)begin
        ...
        for(j=0;j<4;j=j+1)begin 
            sorted_Ai_q[j] <= 0;        sorted_char_q[j]<=0;
            out_attr_arr_C_q[j]<=0;     out_attr_arr_D_temp_q[j]<=0;
        end
    end
    else if(cur_state==EVAL_levelup)begin
        ...
        //case 1-0 find attribute delta (sort, so 2 more cycles, 3 cycles)
        else if(EVAL_levelup_cnt==2 && EVAL_levelup_done==0)begin
            result_A_q<=result_A_d;  //type A output
            for(j=0;j<4;j=j+1)begin 
                ////sort stage 3 (//type B output)
                sorted_Ai_q[j] <= sorted_Ai_d[j];   
                sorted_char_q[j]<=sorted_char_d[j];
                //type C output
                out_attr_arr_C_q[j]<=out_attr_arr_C_d[j];
                //type D output
                out_attr_arr_D_temp_q[j]<=out_attr_arr_D_temp_d[j];
            end
        end
        //case 1-1 find attribute delta
        else if(EVAL_levelup_cnt==3 && EVAL_levelup_done==0)begin
            Delta_MP_q<=Delta_MP;   Delta_HP_q<=Delta_HP;
            Delta_ATK_q<=Delta_ATK; Delta_DEF_q<=Delta_DEF;
        end
        //case 1-2 update attribute
        else if(EVAL_levelup_cnt==4 && EVAL_levelup_done==0)begin
...
always_ff@(posedge clk or negedge inf.rst_n) begin
    if(!inf.rst_n)begin
        //EVAL_skill (2 cycle)
        for(k=0;k<4;k=k+1) sorted_MP_q[k]<=0;
        EVAL_skill_cnt<=0;  EVAL_skill_warn<=No_Warn; EVAL_skill_done<=0;
        //for sort
        for(k=0;k<4;k=k+1)begin
            s1_q[k]<=0; s2_q[k]<=0; s1c_q[k]<=0; s2c_q[k]<=0;
        end
    end
    else if(cur_state==EVAL_levelup)begin
        //case 0 check EXP warn
        //case 1-0 find attribute delta (sort, so 2 more cycles, 3 cycles)
        if(EVAL_levelup_cnt==0 && EVAL_levelup_done==0)begin
            for(k=0;k<4;k=k+1)begin     //sort stage 1
                s1_q[k]<=s1[k];     s1c_q[k]<=s1c[k];
            end 
        end
        else if(EVAL_levelup_cnt==1 && EVAL_levelup_done==0)begin
            for(k=0;k<4;k=k+1)begin     //sort stage 2
                s2_q[k]<=s2[k];     s2c_q[k]<=s2c[k];
            end
        end
        //case 1-1 find attribute delta
        //case 1-2 update attribute
        ...
        else if(EVAL_skill_cnt==2)begin
            for(k=0;k<4;k=k+1) sorted_MP_q[k]<=sorted_Ai_d[k];
        end
        //step 1 find_max skill num with min_mp
        else if(EVAL_skill_cnt==3)begin
            EVAL_skill_done<=1;
            if(can_use_skill_bool==0)EVAL_skill_warn<=MP_Warn;  //can't use any MP
        end
    end
    else if(cur_state==CLEAR)begin
        //EVAL_skill (2 cycle)
...
module find_delta_part1 (
    ...
    output logic [15:0] result_A,   //type A (MP+HP+ATK+DEF)>>3;

    output logic [15:0] out_attr_arr_C_0,
    output logic [15:0] out_attr_arr_C_1,
    output logic [15:0] out_attr_arr_C_2,
    output logic [15:0] out_attr_arr_C_3,

    output logic [15:0] out_attr_arr_D_temp_0,  //type D
    output logic [15:0] out_attr_arr_D_temp_1,
    output logic [15:0] out_attr_arr_D_temp_2,
    output logic [15:0] out_attr_arr_D_temp_3
);
    //type A
    logic [18:0] sum_A;
    assign sum_A=(MP+HP+ATK+DEF);
    assign result_A=(sum_A>>3);

    //type C    //MP, HP, ATK, DEF
    always_comb begin
        if(MP<16383) out_attr_arr_C_0=16383-MP;
        else out_attr_arr_C_0=0;
        if(HP<16383) out_attr_arr_C_1=16383-HP;
        else out_attr_arr_C_1=0;
        if(ATK<16383) out_attr_arr_C_2=16383-ATK;
        else out_attr_arr_C_2=0;
        if(DEF<16383) out_attr_arr_C_3=16383-DEF;
        else out_attr_arr_C_3=0;
    end

    //type D
    assign out_attr_arr_D_temp_0= 3000+((65535-MP)>>4);
    assign out_attr_arr_D_temp_1= 3000+((65535-HP)>>4);
    assign out_attr_arr_D_temp_2= 3000+((65535-ATK)>>4);
    assign out_attr_arr_D_temp_3= 3000+((65535-DEF)>>4);
endmodule

module find_delta_part2 (
    input  logic [1:0] type_in,
    
    input logic [15:0] result_A,   //type A (MP+HP+ATK+DEF)>>3;
    input logic [15:0] sorted_A0,  //type B
    input logic [15:0] sorted_A1,
    input logic [15:0] sorted_A2,
    input logic [15:0] sorted_A3,

    input logic [1:0]  char_A0,
    input logic [1:0]  char_A1,
    input logic [1:0]  char_A2,
    input logic [1:0]  char_A3,

    input logic [15:0] out_attr_arr_C_0,
    input logic [15:0] out_attr_arr_C_1,
    input logic [15:0] out_attr_arr_C_2,
    input logic [15:0] out_attr_arr_C_3,

    input logic [15:0] out_attr_arr_D_temp_0,  //type D
    input logic [15:0] out_attr_arr_D_temp_1,
    input logic [15:0] out_attr_arr_D_temp_2,
    input logic [15:0] out_attr_arr_D_temp_3,
    ...
    always_comb begin
        A2_minus_A0 = sorted_A2 - sorted_A0;
        A3_minus_A1 = sorted_A3 - sorted_A1;
        out_attr_arr_B[0] = 0;  
        out_attr_arr_B[1] = 0;
        out_attr_arr_B[2] = 0;
        out_attr_arr_B[3] = 0;
    ...
    always_comb begin
        out_attr_arr_C[0]=out_attr_arr_C_0;
        out_attr_arr_C[1]=out_attr_arr_C_1;
        out_attr_arr_C[2]=out_attr_arr_C_2;
        out_attr_arr_C[3]=out_attr_arr_C_3;
    end
    //type D
    logic [15:0] out_attr_arr_D [0:3];
    assign out_attr_arr_D[0]=(out_attr_arr_D_temp_0>5047)? 5047:out_attr_arr_D_temp_0;
    assign out_attr_arr_D[1]=(out_attr_arr_D_temp_1>5047)? 5047:out_attr_arr_D_temp_1;
    assign out_attr_arr_D[2]=(out_attr_arr_D_temp_2>5047)? 5047:out_attr_arr_D_temp_2;
    assign out_attr_arr_D[3]=(out_attr_arr_D_temp_3>5047)? 5047:out_attr_arr_D_temp_3;
    //out sig
    always_comb begin
        case(type_in)
            0:begin
