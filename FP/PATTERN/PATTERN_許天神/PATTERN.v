...
parameter DUMP_MODE   = 1;
parameter DEBUG       = 3;
parameter DEBUG_PIXEL = 0;
parameter SEED        = 100;
...
task YOU_PASS_task;
    begin
        avg = total_latency * 1.0 / (PATNUM*64);
        $display("Your Average execution cycles = %.3f cycles", avg);
        $display("----------------------------------------------------------------------------");
        $display("                            Congratulations!");
        $display("                      You have passed all patterns!");
        $display("              Your total execution cycles = %5d cycles ", total_latency);
        $display("            Your Average execution cycles = %.3f cycles ", avg);
        $display("----------------------------------------------------------------------------");
        $finish;
    end
...
begin
    ...
    for (i=0; i<offset; i=i+1) begin
        base = $random(seed_local);
        if (base < 0) base = -base;
        base = base % 117;
        instruction[i][8:1] = base & 8'hFF;

        inc = $random(seed_local);
        if (inc < 0) inc = -inc;
        inc = inc % 6;
        ...
        if (temp > 117)
            instruction[i+offset][8:1] = (temp - 80) & 8'hFF;
        else
            instruction[i+offset][8:1] = temp & 8'hFF;

        instruction[i][0]        = $random(seed_local) & 1;
        instruction[i+offset][0] = $random(seed_local) & 1;
    end

    r = $random(seed_local);
    if (r < 0) r = -r;
    wait_n = (r % 4) + 3;
    repeat(wait_n) @(negedge clk);
...
task dump_BI_matrix;
    integer r, c;
    integer v, iter, img_i, ord_i;
begin
    for (iter = 0; iter < 4; iter = iter + 1) begin
        img_i = iter % 2;          // 0: L0, 1: L1
        ord_i     = iter / 2;                 // use order as point index (0/1)
        $display("==============================================");
