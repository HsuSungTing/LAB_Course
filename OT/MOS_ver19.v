...
always@(*)begin
    case (cur_state)
        ...
        EVAL: begin
            if(global_cnt>=20) next_state=OUTPUT;
...
//8x8 PE
reg signed [15:0] a_in_d[0:7][0:7];
reg signed [15:0] b_in_d[0:7][0:7];
reg signed [31:0] last_mul_result[0:7][0:7];
reg signed [39:0] cur_sum[0:7][0:7];

//Part 2 Comb Logic
reg signed [15:0] a_in [0:7][0:7];
reg signed [15:0] b_in [0:7][0:7];
wire signed [31:0] mul_out [0:7][0:7];
...
generate
  for(mul_i = 0; mul_i < 8; mul_i = mul_i + 1) begin : mul_i_block
    for(mul_j = 0; mul_j < 8; mul_j = mul_j + 1) begin : mul_j_block
      Comb_MUL mul_inst (
        .a_in(a_in_d[mul_i][mul_j]),
        .b_in(b_in_d[mul_i][mul_j]),
  ...
  if(!rst_n)begin
    for(i=0;i<8;i=i+1)begin
      for(j=0;j<8;j=j+1)begin
        a_in_d[i][j]<=16'd0; b_in_d[i][j]<=16'd0;
      end
    end 
  end
  else if(cur_state==CLEAR)begin
    ...
    if(global_cnt>=0&&global_cnt<=21)begin
      for(i=0;i<8;i=i+1)begin
        for(j=0;j<8;j=j+1)begin
          a_in_d[i][j]<=a_in[i][j]; b_in_d[i][j]<=b_in[i][j];  
        end
      end 
    end
  end
...
always@(posedge clk or negedge rst_n)begin
  if(!rst_n)begin
    ...
    if(global_cnt>=2&&global_cnt<=23)begin
      for(i=0;i<8;i=i+1)begin
        for(j=0;j<8;j=j+1)begin
...
always@(posedge clk or negedge rst_n)begin
  if(!rst_n)begin
    ...
    //15 cycle
    // diagnal_sum[0]
    if(global_cnt==10) diagnal_sum[0] <= cur_sum[0][0];
    else diagnal_sum[0] <= diagnal_sum[0];

    // diagnal_sum[1]
    if(global_cnt==11) diagnal_sum[1] <= cur_sum[0][1];
    else if(global_cnt==12) diagnal_sum[1] <= diagnal_sum[1] + cur_sum[1][0];
    else diagnal_sum[1] <= diagnal_sum[1];

    // diagnal_sum[2]
    if(global_cnt==12) diagnal_sum[2] <= cur_sum[0][2];
    else if(global_cnt==13) diagnal_sum[2] <= diagnal_sum[2] + cur_sum[1][1];
    else if(global_cnt==14) diagnal_sum[2] <= diagnal_sum[2] + cur_sum[2][0];
    else diagnal_sum[2] <= diagnal_sum[2];

    // diagnal_sum[3]
    if(global_cnt==13) diagnal_sum[3] <= cur_sum[0][3];
    else if(global_cnt==14) diagnal_sum[3] <= diagnal_sum[3] + cur_sum[1][2];
    else if(global_cnt==15) diagnal_sum[3] <= diagnal_sum[3] + cur_sum[2][1];
    else if(global_cnt==16) diagnal_sum[3] <= diagnal_sum[3] + cur_sum[3][0];
    else diagnal_sum[3] <= diagnal_sum[3];

    // diagnal_sum[4]
    if(global_cnt==14) diagnal_sum[4] <= cur_sum[0][4];
    else if(global_cnt==15) diagnal_sum[4] <= diagnal_sum[4] + cur_sum[1][3];
    else if(global_cnt==16) diagnal_sum[4] <= diagnal_sum[4] + cur_sum[2][2];
    else if(global_cnt==17) diagnal_sum[4] <= diagnal_sum[4] + cur_sum[3][1];
    else if(global_cnt==18) diagnal_sum[4] <= diagnal_sum[4] + cur_sum[4][0];
    else diagnal_sum[4] <= diagnal_sum[4];

    // diagnal_sum[5]
    if(global_cnt==15) diagnal_sum[5] <= cur_sum[0][5];
    else if(global_cnt==16) diagnal_sum[5] <= diagnal_sum[5] + cur_sum[1][4];
    else if(global_cnt==17) diagnal_sum[5] <= diagnal_sum[5] + cur_sum[2][3];
    else if(global_cnt==18) diagnal_sum[5] <= diagnal_sum[5] + cur_sum[3][2];
    else if(global_cnt==19) diagnal_sum[5] <= diagnal_sum[5] + cur_sum[4][1];
    else if(global_cnt==20) diagnal_sum[5] <= diagnal_sum[5] + cur_sum[5][0];
    else diagnal_sum[5] <= diagnal_sum[5];

    // diagnal_sum[6]
    if(global_cnt==16) diagnal_sum[6] <= cur_sum[0][6];
    else if(global_cnt==17) diagnal_sum[6] <= diagnal_sum[6] + cur_sum[1][5];
    else if(global_cnt==18) diagnal_sum[6] <= diagnal_sum[6] + cur_sum[2][4];
    else if(global_cnt==19) diagnal_sum[6] <= diagnal_sum[6] + cur_sum[3][3];
    else if(global_cnt==20) diagnal_sum[6] <= diagnal_sum[6] + cur_sum[4][2];
    else if(global_cnt==21) diagnal_sum[6] <= diagnal_sum[6] + cur_sum[5][1];
    else if(global_cnt==22) diagnal_sum[6] <= diagnal_sum[6] + cur_sum[6][0];
    else diagnal_sum[6] <= diagnal_sum[6];

    // diagnal_sum[7]
    if(global_cnt==17) diagnal_sum[7] <= cur_sum[0][7];
    else if(global_cnt==18) diagnal_sum[7] <= diagnal_sum[7] + cur_sum[1][6];
    else if(global_cnt==19) diagnal_sum[7] <= diagnal_sum[7] + cur_sum[2][5];
    else if(global_cnt==20) diagnal_sum[7] <= diagnal_sum[7] + cur_sum[3][4];
    else if(global_cnt==21) diagnal_sum[7] <= diagnal_sum[7] + cur_sum[4][3];
    else if(global_cnt==22) diagnal_sum[7] <= diagnal_sum[7] + cur_sum[5][2];
    else if(global_cnt==23) diagnal_sum[7] <= diagnal_sum[7] + cur_sum[6][1];
    else if(global_cnt==24) diagnal_sum[7] <= diagnal_sum[7] + cur_sum[7][0];
    else diagnal_sum[7] <= diagnal_sum[7];

    // diagnal_sum[8]
    if(global_cnt==18) diagnal_sum[8] <= cur_sum[1][7];
    else if(global_cnt==19) diagnal_sum[8] <= diagnal_sum[8] + cur_sum[2][6];
    else if(global_cnt==20) diagnal_sum[8] <= diagnal_sum[8] + cur_sum[3][5];
    else if(global_cnt==21) diagnal_sum[8] <= diagnal_sum[8] + cur_sum[4][4];
    else if(global_cnt==22) diagnal_sum[8] <= diagnal_sum[8] + cur_sum[5][3];
    else if(global_cnt==23) diagnal_sum[8] <= diagnal_sum[8] + cur_sum[6][2];
    else if(global_cnt==24) diagnal_sum[8] <= diagnal_sum[8] + cur_sum[7][1];
    else diagnal_sum[8] <= diagnal_sum[8];

    // diagnal_sum[9]
    if(global_cnt==19) diagnal_sum[9] <= cur_sum[2][7];
    else if(global_cnt==20) diagnal_sum[9] <= diagnal_sum[9] + cur_sum[3][6];
    else if(global_cnt==21) diagnal_sum[9] <= diagnal_sum[9] + cur_sum[4][5];
    else if(global_cnt==22) diagnal_sum[9] <= diagnal_sum[9] + cur_sum[5][4];
    else if(global_cnt==23) diagnal_sum[9] <= diagnal_sum[9] + cur_sum[6][3];
    else if(global_cnt==24) diagnal_sum[9] <= diagnal_sum[9] + cur_sum[7][2];
    else diagnal_sum[9] <= diagnal_sum[9];

    // diagnal_sum[10]
    if(global_cnt==20) diagnal_sum[10] <= cur_sum[3][7];
    else if(global_cnt==21) diagnal_sum[10] <= diagnal_sum[10] + cur_sum[4][6];
    else if(global_cnt==22) diagnal_sum[10] <= diagnal_sum[10] + cur_sum[5][5];
    else if(global_cnt==23) diagnal_sum[10] <= diagnal_sum[10] + cur_sum[6][4];
    else if(global_cnt==24) diagnal_sum[10] <= diagnal_sum[10] + cur_sum[7][3];
    else diagnal_sum[10] <= diagnal_sum[10];

    // diagnal_sum[11]
    if(global_cnt==21) diagnal_sum[11] <= cur_sum[4][7];
    else if(global_cnt==22) diagnal_sum[11] <= diagnal_sum[11] + cur_sum[5][6];
    else if(global_cnt==23) diagnal_sum[11] <= diagnal_sum[11] + cur_sum[6][5];
    else if(global_cnt==24) diagnal_sum[11] <= diagnal_sum[11] + cur_sum[7][4];
    else diagnal_sum[11] <= diagnal_sum[11];

    // diagnal_sum[12]
    if(global_cnt==22) diagnal_sum[12] <= cur_sum[5][7];
    else if(global_cnt==23) diagnal_sum[12] <= diagnal_sum[12] + cur_sum[6][6];
    else if(global_cnt==24) diagnal_sum[12] <= diagnal_sum[12] + cur_sum[7][5];
    else diagnal_sum[12] <= diagnal_sum[12];

    // diagnal_sum[13]
    if(global_cnt==23) diagnal_sum[13] <= cur_sum[6][7];
    else if(global_cnt==24) diagnal_sum[13] <= diagnal_sum[13] + cur_sum[7][6];
    else diagnal_sum[13] <= diagnal_sum[13];

    // diagnal_sum[14]
    if(global_cnt==24) diagnal_sum[14] <= cur_sum[7][7];
