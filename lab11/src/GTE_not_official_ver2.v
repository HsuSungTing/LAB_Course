...
reg [6:0] ms_mem_idx, md_mem_idx;	//Part 2 Comb Logic: EVAL_sram_read
reg [8:0] ms_sram_len;
...
always@(posedge clk or negedge rst_n)begin
	if(!rst_n)begin
		...
		EVAL_sram_read_cnt<=0; read_sram_row<=0; read_sram_col<=0; 
		//sram addr, din, WEB
		for(i=0;i<8;i=i+1) WEB_arr[i]<=1;	//read
		sram_4096_addr<=0; sram_4096_din<=0;
		sram_2048_addr<=0; sram_2048_din<=0;
		write_sram_2048_cnt<=0;
		sram_1024_addr<=0; sram_1024_din<=0;
	...
	//case 1: EVAL_sram_read sram raed
		...
		if(EVAL_sram_read_cnt>=2&& EVAL_sram_read_cnt<=ms_sram_len+1)begin
			//case 0 一次寫一個data
			if(ms_mem_idx>=0 && ms_mem_idx<=3)begin
				if(ms_mem_idx==0)img_reg[read_sram_row][read_sram_col]<=mem0_dout;
				else if(ms_mem_idx==1) img_reg[read_sram_row][read_sram_col]<=mem1_dout;
				else if(ms_mem_idx==2) img_reg[read_sram_row][read_sram_col]<=mem2_dout;
				else if(ms_mem_idx==3) img_reg[read_sram_row][read_sram_col]<=mem3_dout;
			end
			//case 1 一次寫兩個data
			else if(ms_mem_idx==4 || ms_mem_idx==5)begin
				if(ms_mem_idx==4)begin
					img_reg[read_sram_row][read_sram_col]<=mem4_dout[15:8];
					img_reg[read_sram_row][read_sram_col+1]<=mem4_dout[7:0];
				end
				else if(ms_mem_idx==5)begin
					img_reg[read_sram_row][read_sram_col]<=mem5_dout[15:8];
					img_reg[read_sram_row][read_sram_col+1]<=mem5_dout[7:0];
				end
			end
			//case 2 一次寫四個data
			else if(ms_mem_idx==6 || ms_mem_idx==7)begin
				if(ms_mem_idx==6)begin
					img_reg[read_sram_row][read_sram_col]  <=mem6_dout[31:24];
					img_reg[read_sram_row][read_sram_col+1]<=mem6_dout[23:16];
					img_reg[read_sram_row][read_sram_col+2]<=mem6_dout[15:8];
					img_reg[read_sram_row][read_sram_col+3]<=mem6_dout[7:0];
				end
				else begin
					img_reg[read_sram_row][read_sram_col]  <=mem7_dout[31:24];
					img_reg[read_sram_row][read_sram_col+1]<=mem7_dout[23:16];
					img_reg[read_sram_row][read_sram_col+2]<=mem7_dout[15:8];
					img_reg[read_sram_row][read_sram_col+3]<=mem7_dout[7:0];
					img_reg[read_sram_row][read_sram_col+1]<=mem7_dout[7:0];
				end
			end
		end
	end
...
always@(*)begin
	//case 0: INPUT_1 sram write 
	if(in_valid_data==1'b1 || cur_state==INPUT_1)begin
