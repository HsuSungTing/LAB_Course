    ...
    if(md < 16) begin // MEM0: image 0-15
        for(local_i=0; local_i<IMG_SIZE; local_i=local_i+1) begin
            for(local_j=0; local_j<IMG_SIZE; local_j=local_j+1) begin
                ...
                `ifdef RTL
                    result_img[local_i][local_j] = u_GTE.MEM0.Memory[addr];
                `elsif GATE
                    result_img[local_i][local_j] = u_GTE.MEM0.Memory[addr];
                `elsif POST
                    result_img[local_i][local_j] = u_CHIP.MEM0.Memory[addr];
                `endif
            end
        end
    end
        ...
        for(local_i=0; local_i<IMG_SIZE; local_i=local_i+1) begin
            for(local_j=0; local_j<IMG_SIZE; local_j=local_j+1) begin
                ...
                `ifdef RTL
                    result_img[local_i][local_j] = u_GTE.MEM1.Memory[addr];
                `elsif GATE
                    result_img[local_i][local_j] = u_GTE.MEM1.Memory[addr];
                `elsif POST
                    result_img[local_i][local_j] = u_CHIP.MEM1.Memory[addr];
                `endif
            end
        end
    end
        ...
        for(local_i=0; local_i<IMG_SIZE; local_i=local_i+1) begin
            for(local_j=0; local_j<IMG_SIZE; local_j=local_j+1) begin
                addr = img_idx * 256 + local_i * 16 + local_j;
                `ifdef RTL
                    result_img[local_i][local_j] = u_GTE.MEM2.Memory[addr];
                `elsif GATE
                    result_img[local_i][local_j] = u_GTE.MEM2.Memory[addr];
                `elsif POST
                    result_img[local_i][local_j] = u_CHIP.MEM2.Memory[addr];
                `endif
            end
        end
    end
        ...
        for(local_i=0; local_i<IMG_SIZE; local_i=local_i+1) begin
            for(local_j=0; local_j<IMG_SIZE; local_j=local_j+1) begin
                addr = img_idx * 256 + local_i * 16 + local_j;
                `ifdef RTL
                    result_img[local_i][local_j] = u_GTE.MEM3.Memory[addr];
                `elsif GATE
                    result_img[local_i][local_j] = u_GTE.MEM3.Memory[addr];
                `elsif POST
                    result_img[local_i][local_j] = u_CHIP.MEM3.Memory[addr];
                `endif
            end
        end
    end
        ...
        for(local_i=0; local_i<IMG_SIZE; local_i=local_i+1) begin
            for(local_j=0; local_j<IMG_SIZE; local_j=local_j+2) begin
                ...
                `ifdef RTL
                    {result_img[local_i][local_j], result_img[local_i][local_j+1]} = u_GTE.MEM4.Memory[addr];
                `elsif GATE
                    {result_img[local_i][local_j], result_img[local_i][local_j+1]} = u_GTE.MEM4.Memory[addr];
                `elsif POST
                    {result_img[local_i][local_j], result_img[local_i][local_j+1]} = u_CHIP.MEM4.Memory[addr];
                `endif
            end
        end
    end
        ...
        for(local_i=0; local_i<IMG_SIZE; local_i=local_i+1) begin
            for(local_j=0; local_j<IMG_SIZE; local_j=local_j+2) begin
                addr = img_idx * 128 + local_i * 8 + local_j/2;
                `ifdef RTL
                    {result_img[local_i][local_j], result_img[local_i][local_j+1]} = u_GTE.MEM5.Memory[addr];
                `elsif GATE
                    {result_img[local_i][local_j], result_img[local_i][local_j+1]} = u_GTE.MEM5.Memory[addr];
                `elsif POST
                    {result_img[local_i][local_j], result_img[local_i][local_j+1]} = u_CHIP.MEM5.Memory[addr];
                `endif
            end
        end
    end
        ...
        for(local_i=0; local_i<IMG_SIZE; local_i=local_i+1) begin
            for(local_j=0; local_j<IMG_SIZE; local_j=local_j+4) begin
                ...
                `ifdef RTL
                    {result_img[local_i][local_j], result_img[local_i][local_j+1], result_img[local_i][local_j+2], result_img[local_i][local_j+3]} = u_GTE.MEM6.Memory[addr];
                `elsif GATE
                    {result_img[local_i][local_j], result_img[local_i][local_j+1], result_img[local_i][local_j+2], result_img[local_i][local_j+3]} = u_GTE.MEM6.Memory[addr];
                `elsif POST
                    {result_img[local_i][local_j], result_img[local_i][local_j+1], result_img[local_i][local_j+2], result_img[local_i][local_j+3]} = u_CHIP.MEM6.Memory[addr];
                `endif
            end
        end
    end
        ...
        for(local_i=0; local_i<IMG_SIZE; local_i=local_i+1) begin
            for(local_j=0; local_j<IMG_SIZE; local_j=local_j+4) begin
                addr = img_idx * 64 + local_i * 4 + local_j/4;
                `ifdef RTL
                    {result_img[local_i][local_j], result_img[local_i][local_j+1], result_img[local_i][local_j+2], result_img[local_i][local_j+3]} = u_GTE.MEM7.Memory[addr];
                `elsif GATE
                    {result_img[local_i][local_j], result_img[local_i][local_j+1], result_img[local_i][local_j+2], result_img[local_i][local_j+3]} = u_GTE.MEM7.Memory[addr];
                `elsif POST
                    {result_img[local_i][local_j], result_img[local_i][local_j+1], result_img[local_i][local_j+2], result_img[local_i][local_j+3]} = u_CHIP.MEM7.Memory[addr];
                `endif
            end
        end
    end
