...
integer i, j, k;
integer pat;
integer total_pat = 100; // Testing patterns count
integer gap;
integer seed = 123;
    ...
    for (r = 0; r < 16; r = r + 1) begin
        for (c = 0; c < 16; c = c + 1) begin
            ...
            if (sram_val !== golden_data[md][r][c]) begin
                ...
                $display("  Expected: %h, Got: %h", golden_data[md][r][c], sram_val);
                $display("--------------------------------------");
                $finish;
            end
        end
...
function [7:0] get_sram_data;
    ...
    begin
        ...
        `elsif GATE
            `define MEM_PATH(mem) TESTBED.u_GTE.mem.Memory
        `elsif POST
            `define MEM_PATH(mem) TESTBED.u_CHIP.mem.Memory
        `endif
