...
reg [1:0] HN_col_blk_idx, HN_row_blk_idx;   //兩個都是0~2
...
always@(*)begin
    if(EVAL_HN_tag==2'd1)begin      //去承接目前EVAL_HN在比較的row
        ...
        for(i=0;i<9;i=i+1)begin
            HN_check_cand[i]=9'b0;
        end
    end
end

//===for debug===
// reg [3:0] debug_row [0:8];
// reg [3:0] debug_col [0:8];
// always@(*)begin
//     debug_row[0]=(3*HN_row_idx); debug_col [0]=(3*HN_col_idx);
//     debug_row[1]=(3*HN_row_idx); debug_col [1]=(3*HN_col_idx)+2'd1;
//     debug_row[2]=(3*HN_row_idx); debug_col [2]=(3*HN_col_idx)+2'd2;
//     debug_row[3]=(3*HN_row_idx)+2'd1; debug_col [3]=(3*HN_col_idx);
//     debug_row[4]=(3*HN_row_idx)+2'd1; debug_col [4]=(3*HN_col_idx)+2'd1;
//     debug_row[5]=(3*HN_row_idx)+2'd1; debug_col [5]=(3*HN_col_idx)+2'd2;
//     debug_row[6]=(3*HN_row_idx)+2'd2; debug_col [6]=(3*HN_col_idx);
//     debug_row[7]=(3*HN_row_idx)+2'd2; debug_col [7]=(3*HN_col_idx)+2'd1;
//     debug_row[8]=(3*HN_row_idx)+2'd2; debug_col [8]=(3*HN_col_idx)+2'd2;
// end
//=============== 
...
always@(*)begin
    for(i=0;i<9;i=i+1)begin
        for(j=0;j<9;j=j+1)begin  //令k為[i][j]的blk index
            if(only_cand_ele_idx[0]!=4'd15&&(i==only_cand_row[0]&&j==only_cand_col[0]))updated_cand_map[i][j]=9'd0;
            else if(only_cand_ele_idx[1]!=4'd15&&(i==only_cand_row[1]&&j==only_cand_col[1]))updated_cand_map[i][j]=9'd0;
            else if(only_cand_ele_idx[2]!=4'd15&&(i==only_cand_row[2]&&j==only_cand_col[2]))updated_cand_map[i][j]=9'd0;
            else if(only_cand_ele_idx[3]!=4'd15&&(i==only_cand_row[3]&&j==only_cand_col[3]))updated_cand_map[i][j]=9'd0;
            else if(only_cand_ele_idx[4]!=4'd15&&(i==only_cand_row[4]&&j==only_cand_col[4]))updated_cand_map[i][j]=9'd0;
            else if(only_cand_ele_idx[5]!=4'd15&&(i==only_cand_row[5]&&j==only_cand_col[5]))updated_cand_map[i][j]=9'd0;
            else if(only_cand_ele_idx[6]!=4'd15&&(i==only_cand_row[6]&&j==only_cand_col[6]))updated_cand_map[i][j]=9'd0;
            else if(only_cand_ele_idx[7]!=4'd15&&(i==only_cand_row[7]&&j==only_cand_col[7]))updated_cand_map[i][j]=9'd0;
            else if(only_cand_ele_idx[8]!=4'd15&&(i==only_cand_row[8]&&j==only_cand_col[8]))updated_cand_map[i][j]=9'd0;
            else begin
                updated_cand_map[i][j]= candidate_map[i][j];
                //only_cand_ele_idx[0]是數值
                if(only_cand_ele_idx[0]!=4'd15&&(i==only_cand_row[0]||j==only_cand_col[0]||cand_map_blk_dix[i][j]==only_cand_blk[0]))
                    updated_cand_map[i][j][only_cand_ele_idx[0]-1'd1]=1'b0; // 只要有共同candidate就洗掉
                if(only_cand_ele_idx[1]!=4'd15&&(i==only_cand_row[1]||j==only_cand_col[1]||cand_map_blk_dix[i][j]==only_cand_blk[1]))
                    updated_cand_map[i][j][only_cand_ele_idx[1]-1'd1]=1'b0; // 只要有共同candidate就洗掉
                if(only_cand_ele_idx[2]!=4'd15&&(i==only_cand_row[2]||j==only_cand_col[2]||cand_map_blk_dix[i][j]==only_cand_blk[2]))
                    updated_cand_map[i][j][only_cand_ele_idx[2]-1'd1]=1'b0; // 只要有共同candidate就洗掉
                if(only_cand_ele_idx[3]!=4'd15&&(i==only_cand_row[3]||j==only_cand_col[3]||cand_map_blk_dix[i][j]==only_cand_blk[3]))
                    updated_cand_map[i][j][only_cand_ele_idx[3]-1'd1]=1'b0; // 只要有共同candidate就洗掉
                if(only_cand_ele_idx[4]!=4'd15&&(i==only_cand_row[4]||j==only_cand_col[4]||cand_map_blk_dix[i][j]==only_cand_blk[4]))
                    updated_cand_map[i][j][only_cand_ele_idx[4]-1'd1]=1'b0; // 只要有共同candidate就洗掉
                if(only_cand_ele_idx[5]!=4'd15&&(i==only_cand_row[5]||j==only_cand_col[5]||cand_map_blk_dix[i][j]==only_cand_blk[5]))
                    updated_cand_map[i][j][only_cand_ele_idx[5]-1'd1]=1'b0; // 只要有共同candidate就洗掉
                if(only_cand_ele_idx[6]!=4'd15&&(i==only_cand_row[6]||j==only_cand_col[6]||cand_map_blk_dix[i][j]==only_cand_blk[6]))
                    updated_cand_map[i][j][only_cand_ele_idx[6]-1'd1]=1'b0; // 只要有共同candidate就洗掉
                if(only_cand_ele_idx[7]!=4'd15&&(i==only_cand_row[7]||j==only_cand_col[7]||cand_map_blk_dix[i][j]==only_cand_blk[7]))
                    updated_cand_map[i][j][only_cand_ele_idx[7]-1'd1]=1'b0; // 只要有共同candidate就洗掉
                if(only_cand_ele_idx[8]!=4'd15&&(i==only_cand_row[8]||j==only_cand_col[8]||cand_map_blk_dix[i][j]==only_cand_blk[8]))
                    updated_cand_map[i][j][only_cand_ele_idx[8]-1'd1]=1'b0; // 只要有共同candidate就洗掉
            end
        end
    end
end
...
//Part 2 Comb Logic
integer m,n;
reg [8:0] update_map_single[0:8][0:8];
always@(*)begin
    for(i=0;i<9;i=i+1)begin
        for(j=0;j<9;j=j+1)begin
            //目前已經填完了(9'b0就是不需要candidate)
            if(single_cand[i][j]!=4'd0)update_map_single[i][j]=4'd0;
            else begin
                update_map_single[i][j]=candidate_map[i][j];
                for(m=0;m<9;m=m+1)begin
                    for(n=0;n<9;n=n+1)begin
                        if(single_cand[m][n]!=4'd0 &&
                        (m==i || n==j || cand_map_blk_dix[m][n]==cand_map_blk_dix[i][j]))begin
                            update_map_single[i][j][single_cand[m][n]-1'b1]=1'b0;
                        end
                    end
                end
            end
...
always@(posedge clk or negedge rst_n)begin
    ...
    //case 4 做完一次
        ...
        for(i=0; i<9; i=i+1)begin
            for(j=0; j<9; j=j+1)begin   //如果目前有確定只有一個解的格子就填入
                candidate_map[i][j]<=update_map_single[i][j];
            end
        end
    end
    else if(cur_state==EVAL_HN && EVAL_HN_tag==2'd1 && HN_row_idx<=4'd8)begin
        //case 2: check_row_HN
        if(only_cand_ele_idx[0]!=4'd15)begin map[only_cand_row[0]][only_cand_col[0]]<=only_cand_ele_idx[0]; end //唯一候選1
        if(only_cand_ele_idx[1]!=4'd15)begin map[only_cand_row[1]][only_cand_col[1]]<=only_cand_ele_idx[1]; end //唯一候選2
        if(only_cand_ele_idx[2]!=4'd15)begin map[only_cand_row[2]][only_cand_col[2]]<=only_cand_ele_idx[2]; end //唯一候選3
        if(only_cand_ele_idx[3]!=4'd15)begin map[only_cand_row[3]][only_cand_col[3]]<=only_cand_ele_idx[3]; end //唯一候選4
        if(only_cand_ele_idx[4]!=4'd15)begin map[only_cand_row[4]][only_cand_col[4]]<=only_cand_ele_idx[4]; end //唯一候選5
        if(only_cand_ele_idx[5]!=4'd15)begin map[only_cand_row[5]][only_cand_col[5]]<=only_cand_ele_idx[5]; end //唯一候選6
        if(only_cand_ele_idx[6]!=4'd15)begin map[only_cand_row[6]][only_cand_col[6]]<=only_cand_ele_idx[6]; end //唯一候選7
        if(only_cand_ele_idx[7]!=4'd15)begin map[only_cand_row[7]][only_cand_col[7]]<=only_cand_ele_idx[7]; end //唯一候選8
        if(only_cand_ele_idx[8]!=4'd15)begin map[only_cand_row[8]][only_cand_col[8]]<=only_cand_ele_idx[8]; end //唯一候選9
