...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        ...
        HN_row_blk_idx<=2'd0; HN_col_blk_idx<=2'd0; empty_cnt<=6'd0; sol_cnt<=6'd0;
    ...
    //case 4 做完一次
        ...
        for(i=0; i<9; i=i+1)begin
            for(j=0; j<9; j=j+1)begin   //如果目前有確定只有一個解的格子就填入
                if(single_cand[i][j]!=4'd0)begin
                    map[i][j]<=single_cand[i][j]; sol_cnt<=sol_cnt+1'd1;
                    candidate_map[i][j]<=9'd0; //目前已經填完了(9'b0就是不需要candidate)
                end
                else begin 
                    map[i][j]<=map[i][j]; sol_cnt<=sol_cnt;
        ...
        if(only_cand_ele_idx[0]!=4'd15)begin map[only_cand_ele_idx[0]][HN_col_idx]<=4'd1; sol_cnt<=sol_cnt+1'd1; end //唯一候選1
        if(only_cand_ele_idx[1]!=4'd15)begin map[only_cand_ele_idx[1]][HN_col_idx]<=4'd2; sol_cnt<=sol_cnt+1'd1; end //唯一候選2
        if(only_cand_ele_idx[2]!=4'd15)begin map[only_cand_ele_idx[2]][HN_col_idx]<=4'd3; sol_cnt<=sol_cnt+1'd1; end //唯一候選3
        if(only_cand_ele_idx[3]!=4'd15)begin map[only_cand_ele_idx[3]][HN_col_idx]<=4'd4; sol_cnt<=sol_cnt+1'd1; end //唯一候選4
        if(only_cand_ele_idx[4]!=4'd15)begin map[only_cand_ele_idx[4]][HN_col_idx]<=4'd5; sol_cnt<=sol_cnt+1'd1; end //唯一候選5
        if(only_cand_ele_idx[5]!=4'd15)begin map[only_cand_ele_idx[5]][HN_col_idx]<=4'd6; sol_cnt<=sol_cnt+1'd1; end //唯一候選6
        if(only_cand_ele_idx[6]!=4'd15)begin map[only_cand_ele_idx[6]][HN_col_idx]<=4'd7; sol_cnt<=sol_cnt+1'd1; end //唯一候選7
        if(only_cand_ele_idx[7]!=4'd15)begin map[only_cand_ele_idx[7]][HN_col_idx]<=4'd8; sol_cnt<=sol_cnt+1'd1; end //唯一候選8
        if(only_cand_ele_idx[8]!=4'd15)begin map[only_cand_ele_idx[8]][HN_col_idx]<=4'd9; sol_cnt<=sol_cnt+1'd1; end //唯一候選9
