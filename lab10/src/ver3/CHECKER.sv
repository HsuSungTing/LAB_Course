...
always_ff @(posedge clk iff inf.type_valid) train_type_q = inf.D.d_type[0];


//coverage
//spec 1,2,3
covergroup cover_group_type_and_mode 
    @(posedge clk iff inf.mode_valid);
    option.at_least = 200 ;     //對整個covergroup下每個 bins(含 cross bins都要至少hit 200次
    train_type: coverpoint train_type_q {
        bins train_type_bins[] = {2'd0, 2'd1, 2'd2, 2'd3};
    }
    mode: coverpoint inf.D.d_mode[0] {
        bins mode_bins[] = {2'b00, 2'b01, 2'b10};
    }
    train_type_X_modee: cross train_type, mode;
endgroup
cover_group_type_and_mode cover_group_type_and_mode_inst =new();


//spec 5 sel_action transition要兩百次
covergroup cover_group_sel_action 
...
endgroup
cover_group_sel_action cover_group_sel_action_inst =new();

//spec 4 player_no的auto bin max
covergroup cover_group_player_no
    @(posedge clk iff inf.player_no_valid);
    option.at_least = 2 ;
    option.auto_bin_max = 256 ;
    player_no: coverpoint inf.D.d_player_no[0];
endgroup
cover_group_player_no cover_group_player_no_inst =new();

//spec 6 MP list的auto bin max
covergroup cover_group_mp
    @(posedge clk iff inf.MP_valid);
    option.at_least = 1 ;
    option.auto_bin_max = 32 ;
    player_no: coverpoint inf.D.d_attribute[0];
endgroup
cover_group_mp cover_group_mp_inst =new();
...
covergroup cover_group_warnmsg 
    ...
    warnmsg: coverpoint inf.warn_msg {
        bins warnmsg_bons[] = {3'h0, 3'h1, 3'h2, 3'h3, 3'h4};
    }
endgroup
