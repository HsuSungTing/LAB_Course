...
//for spec 1,2,3
logic can_sample;
logic [1:0] train_type_q, mode_q;
always_ff @(posedge clk)begin 
    if(inf.type_valid==1) can_sample<=0;
    else if(inf.mode_valid==1) can_sample<=1;
    else can_sample<=can_sample;

    if(inf.type_valid==1) train_type_q=inf.D.d_type[0];  
    if(inf.mode_valid==1) mode_q=inf.D.d_mode[0];
end

//coverage
//spec 1
covergroup cover_group_type
...
endgroup
cover_group_type cover_group_type_inst =new();
