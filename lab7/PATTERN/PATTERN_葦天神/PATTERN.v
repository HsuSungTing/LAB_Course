...
initial begin
    ...
    // Start patterns
    for (pat_cnt = 0; pat_cnt < PATTERN_NUM-1; pat_cnt = pat_cnt + 1) begin
        // Load one pattern's data
        $display("here 0 %0d", pat_cnt);
        load_pattern_task;
        $display("here 1 %0d", pat_cnt);
        // Execute test
        input_task;
        $display("here 2 %0d", pat_cnt);
        wait_output_task;
        $display("here 3 %0d", pat_cnt);
        check_idle_task;
        $display("here 4 %0d", pat_cnt);
...
begin
    ...
    // Check outputs
    output_cnt = 0;
    while (output_cnt < OUTPUT_CYCLE) begin
        //@(negedge clk3);
        if (out_valid === 1) begin
            if (out_data !== current_output[output_cnt]) begin
                $display("========================================");
                $display("  Output mismatch!");
                $display("  Pattern: %0d, Output: %0d", pat_cnt, output_cnt);
                $display("  Golden: 0x%h, Your: 0x%h", 
                         current_output[output_cnt], out_data);
                $display("========================================");
                repeat(2) @(negedge clk1);
                $finish;
            end
