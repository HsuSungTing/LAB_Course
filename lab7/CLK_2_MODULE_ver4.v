...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n) busy<=0;
    else if(cur_state==EVAL || cur_state==OUTPUT || cur_state==CLEAR) busy<=1;
    else busy<=0;
end
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        output_cnt<=8'd0; out_valid<=1'b0; out_data<=16'd0;                
    end
    else if(cur_state==INPUT)begin
        output_cnt<=8'd0; out_valid<=1'b0; out_data<=16'd0;                
    end
    else if(cur_state==OUTPUT && output_cnt<=127 && fifo_full==0)begin
        //not full時才能傳下一筆
        output_cnt<=output_cnt+1'd1; out_valid<=1'b1; out_data<=x_arr[output_cnt];    
    end
    else if(cur_state==OUTPUT && output_cnt<=127 && fifo_full==1)begin
        //full時要暫停輸出
        output_cnt<=output_cnt; out_valid<=1'b0; out_data<=out_data;             
    end
    else begin
        output_cnt<=8'd0; out_valid<=1'b0; out_data<=16'd0;                
    end
