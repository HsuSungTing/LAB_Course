...
always@(*)begin
    case(cur_state)
        ...
        INPUT: begin
            if(input_cnt>=15) next_state=EVAL;
        ...
        OUTPUT: begin
            if(output_cnt >= 127) next_state=CLEAR;
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
    ...
    else if (cur_state==EVAL && EVAL_done==0) begin
        x_arr[cur_j]<=u_plus_v_mod_Q_out;
        x_arr[cur_j+cur_ht]<=u_minus_v_mod_Q_out;
        ...
        //===for EVAL===
        cur_m<=8'd0; cur_i<=8'd0; cur_j<=8'd0;
        cur_t<=8'd128; cur_ht<=7'd0; cur_s<=16'd0; cur_j2<=8'd0;
        EVAL_done<=0;
    end
end
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        output_cnt<=8'd0;   out_valid<=1'b0;
        out_data<=16'd0;    busy<=1'b0;
    end
    else if(cur_state==OUTPUT && output_cnt<=127)begin
        output_cnt<=output_cnt+1'd1;
        out_valid<=1'b1;
        out_data<=x_arr[output_cnt];
        busy<=1'b1;
    end
    else begin
        output_cnt<=8'd0;   out_valid<=1'b0;
...
module u_plus_v_mod_Q (
    ...
    // Q = 12289;
    // sum 最大 131070 -> 17 bits 足夠
    wire [16:0] sum;
    // 顯式把 A/B 擴成 17 bits 再相加，避免 intermediate overflow 被截掉
    assign sum = {1'b0, A} + {1'b0, B};

    always @(*) begin   // 以連續判斷 thresholds: k*Q (k=1..10)
        if (sum < 17'd12289)        out = sum[15:0];
