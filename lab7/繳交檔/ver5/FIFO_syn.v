...
//Part 3 Seq Logic
wire [6:0] rptr_synced, wptr_synced;    
...
reg [6:0] r_binary_q;   //current sram read address (binary)
reg [6:0] r_binary_d;   //next sram read address (binary)
...
reg [6:0] w_binary_q;   //current sram write address (binary)
reg [6:0] w_binary_d;   //next sram write address (binary)
...
always @(posedge wclk or negedge rst_n) begin
    if  (!rst_n) wfull <= 1'b0;
    else if({~wptr_gray_coded[6:5],wptr_gray_coded[4:0]} == rptr_synced) wfull <=1'b1;
    else wfull <= 1'b0;
end

//write_en 
always@(*)begin
    if(winc==1'b0)write_en=1'b1;    //if(winc==1'b0 || wfull==1'b1 )write_en=1'b1;
    else write_en=1'b0;
end
//==========
