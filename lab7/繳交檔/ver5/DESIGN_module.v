...
reg in_valid_q,in_valid_qq,in_valid_qqq;
reg [31:0] in_data_q,in_data_qq,in_data_qqq;

always@(posedge clk or negedge rst_n)begin
    if(!rst_n) begin
        in_valid_q<=0; in_valid_qq<=0; in_valid_qqq<=0;
        in_data_q<=32'd0; in_data_qq<=32'd0; in_data_qqq<=32'd0;
    end
    else begin
        in_valid_q<=in_valid; in_valid_qq<=in_valid_q; in_valid_qqq<=in_valid_qq;
        in_data_q<=in_data; in_data_qq<=in_data_q; in_data_qqq<=in_data_qq;
    end
end
...
always@(*)begin
    case(cur_state)
        IDLE: begin
            if(in_valid_qqq==1'b1)next_state=INPUT;
            else next_state=IDLE;
        end
        INPUT: begin
...
//Part 3 Seq Logic (for pipeline store prev cycle value for next cycle montgomery_mult_part2)
...
always@(posedge clk or negedge rst_n)begin
    ...
    //to avoid take the same data and add input_cnt repeatedly
    else if (input_cnt<=15) begin
        if(in_valid_qqq==1'b1&&can_take_next_input==1)can_take_next_input<=0;
        else if(can_take_next_input==1 && in_valid_qqq==1'b0)can_take_next_input<=1;
        else if(can_take_next_input==0 && in_valid_qqq==1'b0)can_take_next_input<=1;
        else can_take_next_input<=can_take_next_input;

        if(in_valid_qqq==1'b1&&can_take_next_input==1)begin 
            input_cnt<= input_cnt+1'd1;
            x_arr[input_cnt_mul_8]  <={12'd0,in_data_qqq[3:0]};
            x_arr[input_cnt_mul_8+1]<={12'd0,in_data_qqq[7:4]};
            x_arr[input_cnt_mul_8+2]<={12'd0,in_data_qqq[11:8]};
            x_arr[input_cnt_mul_8+3]<={12'd0,in_data_qqq[15:12]};
            x_arr[input_cnt_mul_8+4]<={12'd0,in_data_qqq[19:16]};
            x_arr[input_cnt_mul_8+5]<={12'd0,in_data_qqq[23:20]};
            x_arr[input_cnt_mul_8+6]<={12'd0,in_data_qqq[27:24]};
            x_arr[input_cnt_mul_8+7]<={12'd0,in_data_qqq[31:28]};
        end
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        ...
        if(cur_i==cur_m-1 && cur_j==cur_j2-1) begin //outer loop it update
...
//Part 3 Seq Logic: for OUTPUT (only output when fifo_empty==0)
//OUTPUT must be comb
reg [15:0] out_data_d;
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        output_cnt<=8'd0; out_data_d<=16'd0;               
    end
    else if(cur_state==OUTPUT && output_cnt<=128)begin
        //send next data when not full
        if(fifo_full==1)begin           
            output_cnt<=output_cnt; out_data_d<=out_data_d; 
        end
        else begin                      //0 1 2 not change
