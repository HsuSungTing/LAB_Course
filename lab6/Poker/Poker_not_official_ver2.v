...
always@(*)begin
    //royal flush在花色0
    if(s_flush_num[0][0]==4'd14&&s_flush_exist_bool[0]==1'b0)begin
        royal_flush_bool=1'b1; 
        for(i=0;i<5;i=i+1)final_s_flush_num[i]=s_flush_num[0][i];
    end
    //royal flush在花色1
    else if(s_flush_num[1][0]==4'd14&&s_flush_exist_bool[1]==1'b0)begin
        royal_flush_bool=1'b1;
        for(i=0;i<5;i=i+1)final_s_flush_num[i]=s_flush_num[1][i];
    end
    //royal flush在花色2
    else if(s_flush_num[2][0]==4'd14&&s_flush_exist_bool[2]==1'b0)begin
        royal_flush_bool=1'b1;
        for(i=0;i<5;i=i+1)final_s_flush_num[i]=s_flush_num[2][i];
    end
    //royal flush在花色3
    else if(s_flush_num[3][0]==4'd14&&s_flush_exist_bool[3]==1'b0)begin
        royal_flush_bool=1'b1;
        for(i=0;i<5;i=i+1)final_s_flush_num[i]=s_flush_num[3][i];
    end
    //straight flush在花色0
    else if(straight_flush_bool==1'b1 && s_flush_num[0][0]>=s_flush_num[1][0] &&
    s_flush_num[0][0]>=s_flush_num[2][0]&&s_flush_num[0][0]>=s_flush_num[3][0])begin
        royal_flush_bool=1'b0; 
        for(i=0;i<5;i=i+1)final_s_flush_num[i]=s_flush_num[0][i];
    end
    //straight flush在花色1
    else if(straight_flush_bool==1'b1 && s_flush_num[1][0]>=s_flush_num[0][0] &&
    s_flush_num[1][0]>=s_flush_num[2][0]&&s_flush_num[1][0]>=s_flush_num[3][0])begin
        royal_flush_bool=1'b0; 
        for(i=0;i<5;i=i+1)final_s_flush_num[i]=s_flush_num[1][i];
    end
    //straight flush在花色2
    else if(straight_flush_bool==1'b1 && s_flush_num[2][0]>=s_flush_num[0][0] &&
    s_flush_num[2][0]>=s_flush_num[1][0]&&s_flush_num[2][0]>=s_flush_num[3][0])begin
        royal_flush_bool=1'b0; 
        for(i=0;i<5;i=i+1)final_s_flush_num[i]=s_flush_num[2][i];
    end
    //straight flush在花色3
    else if(straight_flush_bool==1'b1 && s_flush_num[3][0]>=s_flush_num[0][0] &&
    s_flush_num[3][0]>=s_flush_num[1][0]&&s_flush_num[3][0]>=s_flush_num[2][0])begin
        royal_flush_bool=1'b0; 
        for(i=0;i<5;i=i+1)final_s_flush_num[i]=s_flush_num[3][i];
    end
    else begin
