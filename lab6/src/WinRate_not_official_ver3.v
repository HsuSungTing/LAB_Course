...
always@(*)begin
    case(cur_state)
        ...
        EVAL_winrate: begin
            if(EVAL_winrate_done==1'b1) next_state=OUTPUT;
...
//Part 3 Seq Logic
//為了找 cur_pub_3_num, cur_pub_4_num、cur_pub_3_suit, cur_pub_4_suit;
...
always@(posedge clk or negedge rst_n)begin
    ...
    //case 0 把remain_31_card[big_cnt]、remain_31_card[small_cnt]放入Pohker並統計9人的勝率
    else if(cur_state==EVAL_winrate)begin
        //step 1 更新small_cnt、big_cnt
        permute_cnt<=permute_cnt+10'd1;
        ...
        //step 2 累計EVAL_winrate (有考慮平手的情況)
...
//Part 2 Comb Logic
reg [6:0] player_winrate [0:8]; //7 bit score
always@(*)begin
    for(i=0;i<9;i=i+1)begin
        player_winrate[i]=(player_score[i]*100)/(7'd16740);
    end
end
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        ...
        out_win_rate<= {player_winrate[8],player_winrate[7],player_winrate[6],player_winrate[5],
        player_winrate[4],player_winrate[3],player_winrate[2],player_winrate[1],player_winrate[0]};
        out_valid<=1'b1;
    end
    else begin
        out_win_rate<=63'd0; out_valid<=1'b0;
