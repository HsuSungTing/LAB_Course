...
always@(*)begin
    case(cur_state)
        ...
        EVAL_winrate: begin
            if(EVAL_winrate_cnt>=465) next_state=EVAL_mul_div;
...
//===Part 2 Comb Logic===
integer i,j;
wire [5:0] used_card [0:20];
 
genvar uc_i;
generate
    for(uc_i=0;uc_i<18;uc_i=uc_i+1)begin
        assign used_card[uc_i]={in_hole_suit_q[2*(uc_i+1)-1-:2],in_hole_num_q[4*(uc_i+1)-1-:4]};
    end
endgenerate
assign used_card[18]={pub_0_suit,pub_0_num};
assign used_card[19]={pub_1_suit,pub_1_num};
assign used_card[20]={pub_2_suit,pub_2_num};
...
reg [8:0] out_winner_q;
assign win_num= out_winner_q[0]+out_winner_q[1]+out_winner_q[2]+out_winner_q[3]+
    out_winner_q[4]+out_winner_q[5]+out_winner_q[6]+out_winner_q[7]+out_winner_q[8];
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        //EVAL_winrate
        big_cnt<=5'd1; small_cnt<=5'd0; EVAL_winrate_cnt<=9'd0;
    ...
    //case 0 remain_31_card[big_cnt]、remain_31_card[small_cnt] for Poker and keep 9 player win rate
        ...
        //step 0 update small_cnt、big_cnt
        ...
        //step 1 used reg to avoid to long comb path
        if(EVAL_winrate_cnt>=0&&EVAL_winrate_cnt<=464)out_winner_q<=out_winner;
        else out_winner_q<=out_winner_q;

        //step 2 accumulate EVAL_winrate (1 or 2 or 3 or 4 or 9 winner) wait for 1 cycle
        if(EVAL_winrate_cnt>=1&&EVAL_winrate_cnt<=465)begin
            for(i=0;i<9;i=i+1)begin
                if(out_winner_q[i]==1'b1)begin 
        ...
        //step 3 update EVAL_winrate_cnt
        EVAL_winrate_cnt<=EVAL_winrate_cnt+1'd1;
    end
