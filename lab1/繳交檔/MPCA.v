...
wire [15:0] pkt [0:7];       //pkt0 to pkt7
wire [15:0] ori_pkt [0:7];   //pkt0 to pkt7
...
//===sort===
wire signed [6:0] sorted_pkt [0:7]; //(priority score )sorted_pkt [0] priority
wire [2:0] sorted_pkt_id [0:7];     //sorted_pkt_id [0] max priority
wire [1:0] sorted_target_chl [0:7]; //sorted_target_chl[0] max priority
...
//layer7
wire [2:0]chl_cap_out_lyr7 [0:2];//output，lead to RB!!!!!
...
//mask_valid or not
wire mask_valid[0:7];    //0: max priority
...
reg [1:0] chl_to_RB;    
reg [1:0] other_chl_A,other_chl_B;  

always @(*) begin
    if((total_load[0]>=total_load[1])&&(total_load[0]>=total_load[2])) begin 
        chl_to_RB=2'd0; other_chl_A=2'd1; other_chl_B=2'd2;
    end
    else if((total_load[1]>=total_load[0])&&(total_load[1]>=total_load[2]))begin 
...
module allocate_pkt(
    ...
    always@(*)begin
        if(cur_pivot==2'd3)pivot_0=prefer_chl;  
        else pivot_0=cur_pivot;                 
    end
    ...
    always@(*)begin
        ...
        //case 2
        else if(chl_cap[pivot_0]>3'd0)begin  
            ...
            if(pivot_1==2'd0)begin
                target_chl=2'd0; next_pivot=2'd0;    
                chl_0_cap_out=chl_0_cap-3'd1; chl_1_cap_out=chl_1_cap; chl_2_cap_out=chl_2_cap;
            end
            ...
            if(pivot_2==2'd0)begin
                target_chl=2'd0; next_pivot=2'd2;   
                chl_0_cap_out=chl_0_cap-3'd1; chl_1_cap_out=chl_1_cap; chl_2_cap_out=chl_2_cap;
            end
            ...
            if(pivot_0==2'd0)begin
                target_chl=2'd3; next_pivot=2'd2;   
                chl_0_cap_out=chl_0_cap; chl_1_cap_out=chl_1_cap; chl_2_cap_out=chl_2_cap;
            end
            else if(pivot_0==2'd1)begin
                target_chl=2'd3; next_pivot=2'd0;   
                chl_0_cap_out=chl_0_cap; chl_1_cap_out=chl_1_cap; chl_2_cap_out=chl_2_cap;
            end
            else begin  //pivot_2==2'd2
                target_chl=2'd3; next_pivot=2'd1;   
                chl_0_cap_out=chl_0_cap; chl_1_cap_out=chl_1_cap; chl_2_cap_out=chl_2_cap;
            end
        end
    end
...
module merge_sort(
    ...
    input  signed [6:0] in0, in1, in2, in3, in4, in5, in6, in7; 
    input  [2:0]       in_char0, in_char1, in_char2, in_char3, in_char4, in_char5, in_char6, in_char7; 
