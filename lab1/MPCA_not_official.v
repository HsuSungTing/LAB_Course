...
//===sort===
wire signed [6:0] sorted_pkt [0:7]; //sorted_pkt [0] priority最大
...
module find_priority_score(
    ...
    always@(*)begin
        if(qos[1]==1'b1)qos_extd={5'b11111,qos};
        else qos_extd={5'b00000,qos};

        if(pkt_len[3]==1'b1)pkt_len_extd={3'b111,pkt_len};
        else pkt_len_extd={3'b000,pkt_len};

        if(congestion[1]==1'b1)congestion_extd={5'b11111,congestion};
        else congestion_extd={5'b00000,congestion};

        if(src_hint[2]==1'b1)src_hint_extd={4'b1111,src_hint};
