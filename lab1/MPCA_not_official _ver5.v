...
//===for debug===
reg req_valid_arr[0:7];         //0一樣是priority最高，˙Uˊ樣式priority最低的
reg [1:0] prefer_chl_arr[0:7];  //0一樣是priority最高，˙Uˊ樣式priority最低的
...
find_mask_valid P0_mask(
    .p_score(sorted_pkt[0]),.prefer_ch(prefer_chl_arr[0]),
...
find_mask_valid P1_mask(
    .p_score(sorted_pkt[1]),.prefer_ch(prefer_chl_arr[1]),
...
find_mask_valid P2_mask(
    .p_score(sorted_pkt[2]), .prefer_ch(prefer_chl_arr[2]),
...
find_mask_valid P3_mask(
    .p_score(sorted_pkt[3]), .prefer_ch(prefer_chl_arr[3]),
...
find_mask_valid P4_mask(
    .p_score(sorted_pkt[4]), .prefer_ch(prefer_chl_arr[4]),
...
find_mask_valid P5_mask(
    .p_score(sorted_pkt[5]), .prefer_ch(prefer_chl_arr[5]),
...
find_mask_valid P6_mask(
    .p_score(sorted_pkt[6]), .prefer_ch(prefer_chl_arr[6]),
...
find_mask_valid P7_mask(
    .p_score(sorted_pkt[7]), .prefer_ch(prefer_chl_arr[7]),
...
always@(*)begin
    if(total_load[0]+total_load[0]>total_load[1]+total_load[2])begin
        need_RB_bool=1'b1; chl_to_RB=2'd0;
    end
    else if(total_load[1]+total_load[1]>total_load[0]+total_load[2])begin
        need_RB_bool=1'b1; chl_to_RB=2'd1;
    end
    else if(total_load[2]+total_load[2]>total_load[0]+total_load[1])begin
        need_RB_bool=1'b1; chl_to_RB=2'd2;
    end
    else begin need_RB_bool=1'b0; chl_to_RB=2'd3; end
end
...
always@(*)begin
    ...
    //RB 失敗，把chl_after_RB[victim_pkt_idx]設成2'd11
        ...
        for(i=0;i<8;i=i+1)begin
            if(i==victim_pkt_idx)chl_after_RB[i]=2'b11;
            else chl_after_RB[i]=sorted_target_chl[i];
        end
    end
    else if(RB_valid==1'b1)begin
        for(i=0;i<8;i=i+1)begin
            if(i==victim_pkt_idx)chl_after_RB[i]=dst_ch1;
            else chl_after_RB[i]=sorted_target_chl[i];
        end
    end    
    else begin
...
module div3_case(
    input  [3:0] in_val,   // 4-bit input: 0~15
    output reg [3:0] out_val  // 4-bit output: 0~5
);
...
module mod10_7bit(
    input [6:0] in, output reg [3:0] out
);
always @(*) begin
    if (in < 10) out = in;
    ...
    else if (in < 110) out = in - 100;
    else if (in < 120) out = in - 110;
    else if (in < 127) out = in - 120;
    else out = in - 127;
end
endmodule
