...
.inst_zctr(1'b0),.eqb_inst(),.altb_inst(cmp_1_1_altb),.agtb_inst(),
...
wire [31:0] div_in_out;
DW_fp_div_inst div1(.inst_a(div_in_a),.inst_b(div_in_b),.inst_rnd(3'b0),.z_inst(div_in_out),.status_inst());
...
always@(posedge clk or negedge rst_n)begin
    ...
    else if(conv_cnt==81)begin
        cur_positive_exp<=exp_out;  //e^x1
    end
