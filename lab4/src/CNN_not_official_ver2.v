...
always@(*)begin
    case (cur_state)
        ...
        INPUT: begin
            if(input_cnt==8'd71&&task_number_q==0)next_state=EVAL_conv1;
            else if(input_cnt==8'd71&&task_number_q==1)next_state=EVAL_conv2;
...
//Part 3 Seq Logic
always@(posedge clk or negative rst_n)begin
    if(!rst_n)begin
    end
    else if(in_valid==1'b1&&in_cnt<=8)begin
        ker_ch1[in_cnt][0]<=Kernel_ch1; //ker_ch1_1
        ker_ch2[in_cnt][0]<=Kernel_ch2; //ker_ch2_1
    end
    else if(in_valid==1'b1&&in_cnt<=17)begin
        ker_ch1[in_cnt-9][1]<=Kernel_ch1;   //ker_ch1_2
        ker_ch2[in_cnt-9][1]<=Kernel_ch2;   //ker_ch2_2
    end
end
...
always@(posedge clk or negative rst_n)begin
    ...
    else if(in_valid==1'b1&&in_cnt<=7)begin
        big_wgt[in_cnt][0]<=Weight_Bias;
    end
    else if(in_valid==1'b1&&in_cnt<=15)begin
        big_wgt[in_cnt-8][1]<=Weight_Bias;
    end
    else if(in_valid==1'b1&&in_cnt<=23)begin
        big_wgt[in_cnt-16][2]<=Weight_Bias;
    end
    else if(in_valid==1'b1&&in_cnt<=31)begin
        big_wgt[in_cnt-24][3]<=Weight_Bias;
    end
    else if(in_valid==1'b1&&in_cnt<=39)begin
        big_wgt[in_cnt-31][4]<=Weight_Bias;
    end
    else if(in_valid==1'b1&&in_cnt==40)bias_1<=Weight_Bias;
    else if(in_valid==1'b1&&in_cnt<=45)begin
        small_wgt[in_cnt-41][0]<=Weight_Bias;
    end
    else if(in_valid==1'b1&&in_cnt<=50)begin
        small_wgt[in_cnt-46][1]<=Weight_Bias;
    end
    else if(in_valid==1'b1&&in_cnt<=55)begin
        small_wgt[in_cnt-51][2]<=Weight_Bias;
    end
    else if(in_valid==1'b1&&in_cnt==56)bias_2<=Weight_Bias;
end
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        ...
        if(mode_q==2'b00||mode_q==2'b01)begin   //Rep Pad 
            ...
            img_1[62]<=img_1[46]; img_1[63]<=img_1[44];
        end
    end
end
