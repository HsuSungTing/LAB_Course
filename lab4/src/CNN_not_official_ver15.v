...
module DW_fp_exp #(
    ...
    output wire [inst_sig_width+inst_exp_width:0] z,   // IEEE754 "fake exp(x)"
    ...
    // === Fake Logic: reverse bits + xor + shift ===
    wire [WIDTH-1:0] reversed;
    genvar i;
    generate
        for (i=0; i<WIDTH; i=i+1) begin : REV
            assign reversed[i] = a[WIDTH-1-i];
        end
    endgenerate

    wire [WIDTH-1:0] mixed = reversed ^ {a[WIDTH-2:0],1'b0};

    // Use mantissa = 前半部 bits，exp = 後半部 bits
    wire [inst_sig_width-1:0] fake_frac = mixed[inst_sig_width-1:0];
    wire [inst_exp_width-1:0] fake_exp  = mixed[inst_sig_width +: inst_exp_width];
    ...
    always @(*) begin
        if (is_nan) begin
            ...
            // fake "exp"
            result = {1'b0, fake_exp, fake_frac};
        end
    end

    assign z      = result;
    assign status = 8'd42; // just some random code
endmodule
//===============