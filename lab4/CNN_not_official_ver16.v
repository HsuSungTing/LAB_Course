...
//Part 2 Comb Logic
//task 1 step 2 (把0-17和18-35相加) for A、B、C、D
wire [31:0] add_conv_result;    //18個add的結果(從下面搬過來的)
...
//task 0和task 1的step 1
reg [31:0] conved_img [0:35][0:1];  //task 1時是存A B conved後的結果
reg [31:0] conved_img_C [0:35];
reg [31:0] conved_img_D [0:35];

//step 7 softmax的訊號 (只是寫在這裡，Step 7才會用到)
reg [31:0] exp_of_z[0:2];   //三個分子
reg [31:0] sum_of_exp;      //分母

//===sum fo ch1 & ch2===
//step 1的加法器，和step 3共用 (div_den要存add_conv_result(分母))
reg [31:0] add_conv_a_in, add_conv_b_in;
//wire [31:0] add_conv_result; (前面會用到，所以先搬到前面)
...
always@(*)begin
    if(cur_state==EVAL_task_0&&(conv_cnt<=35||conv_cnt<=71))begin
        ...
        //task 0 step 7 softmax (只會用到一個sum3)
...
//===Task 1 step 3===
//比較16個cycle，找最大result(最後要用signed bit判斷正負，負的就改成0)
//會和task 0 step 1共用sum
//reg [31:0] task1_step3_in_1,task1_step3_in_2,task1_step3_in_3,task1_step3_in_4;
...
always@(*)begin
    if(cur_state==EVAL_task_1&&conv_cnt==84)begin
        ...
        result_sum=sum3_out[3];//和task 0 step 1共用
    end
