...
def task0_compute_and_emit(f_in, f_gold, mode_bits, rng, wb_count=63):
    ...
    for i in range(3):
        for j in range(3):
            write_line(f_in, f"{f32_to_hex(k0_ch1[i,j])} {f32_to_hex(k1_ch1[i,j])}")
    for i in range(3):
        for j in range(3):
            write_line(f_in, f"{f32_to_hex(k0_ch2[i,j])} {f32_to_hex(k1_ch2[i,j])}")
...
def main():
    ...
    ap.add_argument("--cases", type=int, default=2000, help="number of testcases")
    ap.add_argument("--task0-ratio", type=float, default=0.65, help="probability of selecting Task0 for a case")
    ap.add_argument("--seed", type=int, default=3129, help="random seed")
