...
always@(*)begin
    case (cur_state)
        ...
        INPUT: begin
            ...
            else if(input_cnt==8'd35&&task_number_q==1)next_state=EVAL_task_1;
            else next_state=INPUT;
        end
        EVAL_task_0: begin   
            if(conv_cnt>=111)next_state=OUTPUT;
            else next_state=EVAL_task_0;
        end
        EVAL_task_1: begin   
            if(conv_cnt==8'd100)next_state=OUTPUT;
            else next_state=EVAL_task_1;
        end
        OUTPUT: begin
            if(out_cnt>=out_bound)next_state=CLEAR;
...
reg [31:0] ker_ch1[0:8][0:1];   //ker_A Ker_B
reg [31:0] ker_ch2[0:8][0:1];   //ker_C Ker_D

reg [31:0] big_wgt[0:7][0:4];   //fc lay 1
reg [31:0] bias_1, bias_2;
reg [31:0] small_wgt[0:4][0:2]; //fc lay 2

reg [31:0] acted_img [0:7];     //only task 0
reg [31:0] img_after_FC1 [0:4]; //only task 0
reg [31:0] img_after_relu [0:4];//only task 0
reg [31:0] img_after_FC2 [0:2]; //only task 0

reg [3:0] capacity_q;          //only task 1
reg [3:0] capacity_cost_rarr[0:3];//only task 1
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        for(i=0;i<9;i=i+1)begin
            ker_ch1[i][0]<=IEEE_zero; ker_ch1[i][1]<=IEEE_zero;
            ker_ch2[i][0]<=IEEE_zero; ker_ch2[i][1]<=IEEE_zero;
        end
    end
    else if(in_valid==1'b1&&input_cnt<=8)begin
        ker_ch1[input_cnt][0]<=Kernel_ch1; //ker_ch1_1 (Ker_A)
        ker_ch2[input_cnt][0]<=Kernel_ch2; //ker_ch2_1 (Ker_C)
    end
    else if(in_valid==1'b1&&input_cnt<=17)begin
        ker_ch1[input_cnt-9][1]<=Kernel_ch1;   //ker_ch1_2 (Ker_B)
        ker_ch2[input_cnt-9][1]<=Kernel_ch2;   //ker_ch2_2 (Ker_D)
    end
    else if(cur_state==CLEAR)begin
        for(i=0;i<9;i=i+1)begin
            ker_ch1[i][0]<=IEEE_zero; ker_ch1[i][1]<=IEEE_zero;
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        capacity_q<=4'd0;
        for(i=0;i<4;i=i+1)capacity_cost_rarr[i]<=4'd0;
    end
    else if(in_valid==1'b1&&input_cnt==0)capacity_q<=capacity_cost;
    else if(in_valid==1'b1&&input_cnt<=4)capacity_cost_rarr[input_cnt-1]<=capacity_cost;
    else if(cur_state==CLEAR)begin
        capacity_q<=4'd0;
        for(i=0;i<4;i=i+1)capacity_cost_rarr[i]<=4'd0;
    end
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        bias_1<=IEEE_zero; bias_2<=IEEE_zero;
        for(i=0;i<8;i=i+1)begin
            for(j=0;j<5;j=j+1)begin
                big_wgt[i][j]<=IEEE_zero;
            end
        end
        for(i=0;i<5;i=i+1)begin
            for(j=0;j<3;j=j+1)begin
                small_wgt[i][j]<=IEEE_zero;
            end
        end
    end
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        input_cnt<=8'd0; task_number_q<=2'd0; mode_q<=2'b00; out_bound<=2'd0;
        for(i=0;i<64;i=i+1)begin
            img_0[i]<=IEEE_zero; img_1[i]<=IEEE_zero;
        end
    end
        ...
        if(input_cnt==0)begin
            ...
            if(task_number==2'd0)out_bound<=2'd2;   //Task 0
            else out_bound<=2'd0;
        end
        else begin
            task_number_q<=task_number_q; mode_q<=mode_q;
        end
        ...
        if(mode_q==2'b00||mode_q==2'b01)begin   //Rep Pad 
            ...
            img_0[62]<=img_0[46];   img_0[63]<=img_0[45];
        end
    end
    ...
    else if(cur_state==EVAL_task_0&&input_cnt>=8'd72)begin
        //for Rep Pad 
        if(mode_q==2'b00||mode_q==2'b01)begin
            img_1[55]<=img_1[54]; img_1[62]<=img_1[54]; img_1[63]<=img_1[54];
        end
    end
    else if(cur_state==EVAL_task_1&&input_cnt>=36)begin
        //for Rep Pad 
        if(mode_q==2'b00||mode_q==2'b01)begin
            img_0[55]<=img_0[54]; img_0[62]<=img_0[54]; img_0[63]<=img_0[54];
        end
...
//Part 2 Comb Logic
//task 1 step 2 (add 0-17 7 18-35) for A, B, C, D
wire [31:0] add_conv_result;    //18 sum result 
reg [31:0] A_result,B_result,C_result,D_result;
reg [31:0] sum_0_17, sum_18_35;
reg [31:0] add_task1_a_in,add_task1_b_in;
wire [31:0] add_task1_result;
DW_fp_add_inst add_task1_step2(.inst_a(add_task1_a_in),.inst_b(add_task1_b_in),.inst_rnd(3'b0),.z_inst(add_task1_result),.status_inst());

always@(*)begin
    if(cur_state==EVAL_task_1&&conv_cnt>=72&&conv_cnt<=83)begin
        //task 1 step 2
        add_task1_a_in=sum_0_17;
        add_task1_b_in=sum_18_35;
    end
    else begin
        add_task1_a_in=sum_0_17; add_task1_b_in=sum_18_35;
    end
end

always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        sum_0_17<=IEEE_zero; sum_18_35<=IEEE_zero;
        A_result<=IEEE_zero; B_result<=IEEE_zero; 
        C_result<=IEEE_zero; D_result<=IEEE_zero;
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==72)begin
        sum_0_17<=add_conv_result;  //A 0-17
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==73)begin
        sum_18_35<=add_conv_result; //A 18-35
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==74)begin
        A_result<=add_task1_result; //sum A (0-17)(18-35)
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==75)begin
        sum_0_17<=add_conv_result;  //B 0-17
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==76)begin
        sum_18_35<=add_conv_result; //B 18-35
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==77)begin
        B_result<=add_task1_result; //sum B (0-17)(18-35)
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==78)begin
        sum_0_17<=add_conv_result;  //C 0-17
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==79)begin
        sum_18_35<=add_conv_result; //C 18-35
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==80)begin
        C_result<=add_task1_result; //sum C (0-17)(18-35)
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==81)begin
        sum_0_17<=add_conv_result;  //D 0-17
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==82)begin
        sum_18_35<=add_conv_result; //D 18-35
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==83)begin
        D_result<=add_task1_result; //sum D (0-17)(18-35)
    end
    else if(cur_state==CLEAR)begin
        sum_0_17<=IEEE_zero; sum_18_35<=IEEE_zero;
        A_result<=IEEE_zero; B_result<=IEEE_zero; 
...
//task 0 & task 1 step 1
reg [31:0] conved_img [0:35][0:1];  
reg [31:0] conved_img_C [0:35];
reg [31:0] conved_img_D [0:35];

//step 7 softmax
reg [31:0] exp_of_z[0:2];  
reg [31:0] sum_of_exp;      

//===sum fo ch1 & ch2===
//step 1 shared with step 3
reg [31:0] add_conv_a_in, add_conv_b_in;
//wire [31:0] add_conv_result; 
reg [31:0] prev_positive_exp,prev_negative_exp;

always@(*)begin
    if(cur_state==EVAL_task_0&&(conv_cnt<=35||conv_cnt<=71))begin
        //task 0 step 1 conv
        add_conv_a_in=sum3_out[3];  
        add_conv_b_in=sum3_out[7];
    end
    else if(cur_state==EVAL_task_0&&(conv_cnt>=81&&conv_cnt<=98))begin
        //task 0 step 2 act e^x + e^-x
        if(mode_q==2'b00||mode_q==2'b10)begin //tahn
    ...
    else if(cur_state==EVAL_task_0&&(conv_cnt>=99&&conv_cnt<=101))begin
        //task 0 step 4 FC 1
        add_conv_a_in=sum3_out[3]; 
        add_conv_b_in=sum3_out[7];
    end
    else if(cur_state==EVAL_task_0&&(conv_cnt>=103&&conv_cnt<=104))begin
        //task 0 step 6 FC 2
        add_conv_a_in=sum3_out[3]; 
        add_conv_b_in=sum3_out[7];
    end
    else if(cur_state==EVAL_task_1&&(conv_cnt>=72&&conv_cnt<=83))begin
        //task 1 step 2
        add_conv_a_in=sum3_out[3]; 
        add_conv_b_in=sum3_out[7];
    end
    else begin
        add_conv_a_in=sum3_out[3]; add_conv_b_in=sum3_out[7];
    end
end
...
//task 1 step 3
reg [31:0] task1_step3_in_1,task1_step3_in_2,task1_step3_in_3,task1_step3_in_4;
//=================================
...
always@(*)begin
    if(cur_state==EVAL_task_0&&(conv_cnt<=35||conv_cnt<=71))begin
    ...
    else if(cur_state==EVAL_task_1&&(conv_cnt<=35||conv_cnt<=71))begin
        //task 0 step 1 img_0 conv A(0-35) C(36-71)
        sum3_a_in[0]=ch1_mult_out[0]; 
        sum3_b_in[0]=ch1_mult_out[1];
        sum3_c_in[0]=ch1_mult_out[2]; 
        sum3_a_in[1]=ch1_mult_out[3]; 
        ...
        //task 1 step 2 sum A 0-17 (0-8)
        sum3_a_in[0]=conved_img[0][0]; 
        sum3_b_in[0]=conved_img[1][0]; 
        sum3_c_in[0]=conved_img[2][0];  
        sum3_a_in[1]=conved_img[3][0];  
        sum3_b_in[1]=conved_img[4][0]; 
        sum3_c_in[1]=conved_img[5][0];  
        sum3_a_in[2]=conved_img[6][0];  
        sum3_b_in[2]=conved_img[7][0]; 
        sum3_c_in[2]=conved_img[8][0];  
        ...
        //task 1 step 2 sum A 18-35 (18-26)
        sum3_a_in[0]=conved_img[18][0]; 
        sum3_b_in[0]=conved_img[19][0]; 
        sum3_c_in[0]=conved_img[20][0];  
        sum3_a_in[1]=conved_img[21][0];  
        sum3_b_in[1]=conved_img[22][0]; 
        sum3_c_in[1]=conved_img[23][0];  
        sum3_a_in[2]=conved_img[24][0];  
        sum3_b_in[2]=conved_img[25][0]; 
        sum3_c_in[2]=conved_img[26][0];  
        ...
        //task 1 step 2 sum B 0-17 (0-8)
        sum3_a_in[0]=conved_img[0][1]; 
        sum3_b_in[0]=conved_img[1][1]; 
        sum3_c_in[0]=conved_img[2][1];  
        sum3_a_in[1]=conved_img[3][1];  
        sum3_b_in[1]=conved_img[4][1]; 
        sum3_c_in[1]=conved_img[5][1];  
        sum3_a_in[2]=conved_img[6][1];  
        sum3_b_in[2]=conved_img[7][1]; 
        sum3_c_in[2]=conved_img[8][1];  
        ...
        //task 1 step 2 sum B 18-35 (18-26)
        sum3_a_in[0]=conved_img[18][1]; 
        sum3_b_in[0]=conved_img[19][1]; 
        sum3_c_in[0]=conved_img[20][1];  
        sum3_a_in[1]=conved_img[21][1];  
        sum3_b_in[1]=conved_img[22][1]; 
        sum3_c_in[1]=conved_img[23][1];  
        sum3_a_in[2]=conved_img[24][1];  
        sum3_b_in[2]=conved_img[25][1]; 
        sum3_c_in[2]=conved_img[26][1];  
        ...
        //task 1 step 2 sum C 0-17 (0-8)
        sum3_a_in[0]=conved_img_C[0]; 
        sum3_b_in[0]=conved_img_C[1]; 
        sum3_c_in[0]=conved_img_C[2];  
        sum3_a_in[1]=conved_img_C[3];  
        sum3_b_in[1]=conved_img_C[4]; 
        sum3_c_in[1]=conved_img_C[5];  
        sum3_a_in[2]=conved_img_C[6];  
        sum3_b_in[2]=conved_img_C[7]; 
        sum3_c_in[2]=conved_img_C[8];  
        ...
        //task 1 step 2 sum C 18-35 (18-26)
        sum3_a_in[0]=conved_img_C[18]; 
        sum3_b_in[0]=conved_img_C[19]; 
        sum3_c_in[0]=conved_img_C[20];  
        sum3_a_in[1]=conved_img_C[21];  
        sum3_b_in[1]=conved_img_C[22]; 
        sum3_c_in[1]=conved_img_C[23];  
        sum3_a_in[2]=conved_img_C[24];  
        sum3_b_in[2]=conved_img_C[25]; 
        sum3_c_in[2]=conved_img_C[26];  
        ...
        //task 1 step 2 sum D 0-17 (0-8)
        sum3_a_in[0]=conved_img_D[0]; 
        sum3_b_in[0]=conved_img_D[1]; 
        sum3_c_in[0]=conved_img_D[2];  
        sum3_a_in[1]=conved_img_D[3];  
        sum3_b_in[1]=conved_img_D[4]; 
        sum3_c_in[1]=conved_img_D[5];  
        sum3_a_in[2]=conved_img_D[6];  
        sum3_b_in[2]=conved_img_D[7]; 
        sum3_c_in[2]=conved_img_D[8];  
        ...
        //task 1 step 2 sum D 18-35 (18-26)
        sum3_a_in[0]=conved_img_D[18]; 
        sum3_b_in[0]=conved_img_D[19]; 
        sum3_c_in[0]=conved_img_D[20];  
        sum3_a_in[1]=conved_img_D[21];  
        sum3_b_in[1]=conved_img_D[22]; 
        sum3_c_in[1]=conved_img_D[23];  
        sum3_a_in[2]=conved_img_D[24];  
        sum3_b_in[2]=conved_img_D[25]; 
        sum3_c_in[2]=conved_img_D[26];  
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt==108)begin
        //step 7 softmax
        sum3_a_in[0]=exp_of_z[0]; 
        sum3_b_in[0]=exp_of_z[1];
        sum3_c_in[0]=exp_of_z[2]; 
        sum3_a_in[1]=IEEE_zero; 
        sum3_b_in[1]=IEEE_zero; 
        sum3_c_in[1]=IEEE_zero; 
        sum3_a_in[2]=IEEE_zero; 
        sum3_b_in[2]=IEEE_zero; 
        sum3_c_in[2]=IEEE_zero; 

        sum3_a_in[3]=sum3_out[0];
        sum3_b_in[3]=sum3_out[1];
        sum3_c_in[3]=sum3_out[2];
    end
    else if(cur_state==EVAL_task_1&&conv_cnt>=84&&conv_cnt<=99)begin
        //task 1 step 3 
        sum3_a_in[0]=task1_step3_in_1; 
        sum3_b_in[0]=task1_step3_in_2;
        sum3_c_in[0]=task1_step3_in_3; 
        sum3_a_in[1]=task1_step3_in_4; 
        sum3_b_in[1]=IEEE_zero; 
        sum3_c_in[1]=IEEE_zero; 
        sum3_a_in[2]=IEEE_zero; 
        sum3_b_in[2]=IEEE_zero; 
...
always@(*)begin
    if(cur_state==EVAL_task_0&&(conv_cnt<=35||conv_cnt<=71))begin
        ...
        //task 1 step 1 img_0 conv B(0-35), D(36-71)
        sum3_a_in[4]=ch2_mult_out[0]; 
        sum3_b_in[4]=ch2_mult_out[1];
        sum3_c_in[4]=ch2_mult_out[2]; 
        sum3_a_in[5]=ch2_mult_out[3]; 
        ...
        //task 1 step 2 sum A 0-17 (9-17)
        sum3_a_in[4]=conved_img[9][0];
        sum3_b_in[4]=conved_img[10][0];
        sum3_c_in[4]=conved_img[11][0];
        sum3_a_in[5]=conved_img[12][0]; 
        sum3_b_in[5]=conved_img[13][0];
        sum3_c_in[5]=conved_img[14][0]; 
        sum3_a_in[6]=conved_img[15][0]; 
        sum3_b_in[6]=conved_img[16][0];
        sum3_c_in[6]=conved_img[17][0]; 
        ...
        //task 1 step 2 sum A 18-35 (27-35)
        sum3_a_in[4]=conved_img[27][0];
        sum3_b_in[4]=conved_img[28][0];
        sum3_c_in[4]=conved_img[29][0];
        sum3_a_in[5]=conved_img[30][0]; 
        sum3_b_in[5]=conved_img[31][0];
        sum3_c_in[5]=conved_img[32][0]; 
        sum3_a_in[6]=conved_img[33][0]; 
        sum3_b_in[6]=conved_img[34][0];
        sum3_c_in[6]=conved_img[35][0]; 
        ...
        //task 1 step 2 sum B 0-17 (9-17)
        sum3_a_in[4]=conved_img[9][1];
        sum3_b_in[4]=conved_img[10][1];
        sum3_c_in[4]=conved_img[11][1];
        sum3_a_in[5]=conved_img[12][1]; 
        sum3_b_in[5]=conved_img[13][1];
        sum3_c_in[5]=conved_img[14][1]; 
        sum3_a_in[6]=conved_img[15][1]; 
        sum3_b_in[6]=conved_img[16][1];
        sum3_c_in[6]=conved_img[17][1]; 
        ...
        //task 1 step 2 sum B 18-35 (27-35)
        sum3_a_in[4]=conved_img[27][1];
        sum3_b_in[4]=conved_img[28][1];
        sum3_c_in[4]=conved_img[29][1];
        sum3_a_in[5]=conved_img[30][1]; 
        sum3_b_in[5]=conved_img[31][1];
        sum3_c_in[5]=conved_img[32][1]; 
        sum3_a_in[6]=conved_img[33][1]; 
        sum3_b_in[6]=conved_img[34][1];
        sum3_c_in[6]=conved_img[35][1]; 
        ...
        //task 1 step 2 sum C 0-17 (9-17)
        sum3_a_in[4]=conved_img_C[9];
        sum3_b_in[4]=conved_img_C[10];
        sum3_c_in[4]=conved_img_C[11];
        sum3_a_in[5]=conved_img_C[12]; 
        sum3_b_in[5]=conved_img_C[13];
        sum3_c_in[5]=conved_img_C[14]; 
        sum3_a_in[6]=conved_img_C[15]; 
        sum3_b_in[6]=conved_img_C[16];
        sum3_c_in[6]=conved_img_C[17]; 
        ...
        //task 1 step 2 sum C 18-35 (27-35)
        sum3_a_in[4]=conved_img_C[27];
        sum3_b_in[4]=conved_img_C[28];
        sum3_c_in[4]=conved_img_C[29];
        sum3_a_in[5]=conved_img_C[30]; 
        sum3_b_in[5]=conved_img_C[31];
        sum3_c_in[5]=conved_img_C[32]; 
        sum3_a_in[6]=conved_img_C[33]; 
        sum3_b_in[6]=conved_img_C[34];
        sum3_c_in[6]=conved_img_C[35]; 
        ...
        //task 1 step 2 sum D 0-17 (9-17)
        sum3_a_in[4]=conved_img_D[9];
        sum3_b_in[4]=conved_img_D[10];
        sum3_c_in[4]=conved_img_D[11];
        sum3_a_in[5]=conved_img_D[12]; 
        sum3_b_in[5]=conved_img_D[13];
        sum3_c_in[5]=conved_img_D[14]; 
        sum3_a_in[6]=conved_img_D[15]; 
        sum3_b_in[6]=conved_img_D[16];
        sum3_c_in[6]=conved_img_D[17]; 
        ...
        //task 1 step 2 sum D 18-35 (27-35)
        sum3_a_in[4]=conved_img_D[27];
        sum3_b_in[4]=conved_img_D[28];
        sum3_c_in[4]=conved_img_D[29];
        sum3_a_in[5]=conved_img_D[30]; 
        sum3_b_in[5]=conved_img_D[31];
        sum3_c_in[5]=conved_img_D[32]; 
        sum3_a_in[6]=conved_img_D[33]; 
        sum3_b_in[6]=conved_img_D[34];
        sum3_c_in[6]=conved_img_D[35]; 
        ...
        //task 0 step 7 softmax 
        sum3_a_in[4]=IEEE_zero;
        sum3_b_in[4]=IEEE_zero;
        sum3_c_in[4]=IEEE_zero; 
        sum3_a_in[5]=IEEE_zero; 
        sum3_b_in[5]=IEEE_zero;
        sum3_c_in[5]=IEEE_zero; 
        sum3_a_in[6]=IEEE_zero; 
        sum3_b_in[6]=IEEE_zero;
        sum3_c_in[6]=IEEE_zero; 

        sum3_a_in[7]=IEEE_zero; 
        sum3_b_in[7]=IEEE_zero; 
        sum3_c_in[7]=IEEE_zero; 
    end
    else begin
        sum3_a_in[4]=ch2_mult_out[0]; 
        sum3_b_in[4]=ch2_mult_out[1];
...
//===Part 3 Seq Logic===
//reg [31:0] conved_img [0:35][0:1];  //task 1 store A B conved
//reg [31:0] conved_img_C [0:35];
//reg [31:0] conved_img_D [0:35];
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        for(i=0;i<36;i=i+1)begin
            conved_img [i][0]<=IEEE_zero; conved_img [i][1]<=IEEE_zero;
            conved_img_C[i]<=IEEE_zero; conved_img_D[i]<=IEEE_zero;
        end
    end
    else if(cur_state==EVAL_task_0&&(conv_cnt<=35))begin
        //task 0 step 1
        conved_img[conv_cnt][0]<=add_conv_result;
    end
    else if(cur_state==EVAL_task_0&&(conv_cnt<=71))begin
        //task 0 step 1
        conved_img[conv_cnt-36][1]<=add_conv_result;
    end
    else if(cur_state==EVAL_task_1&&(conv_cnt<=35))begin
        //task 1 step 1 conv
        conved_img[conv_cnt][0]<=sum3_out[3];
        conved_img[conv_cnt][1]<=sum3_out[7];
    end
    else if(cur_state==EVAL_task_1&&(conv_cnt<=71))begin
        //task 1 step 1 conv
        conved_img_C[conv_cnt-36]<=sum3_out[3];
        conved_img_D[conv_cnt-36]<=sum3_out[7];
    end
    else if(cur_state==CLEAR)begin
        for(i=0;i<36;i=i+1)begin
            conved_img [i][0]<=IEEE_zero; conved_img [i][1]<=IEEE_zero;
...
//Part 2 Comb Logic mult_in_a 
always@(*)begin
    if(cur_state==EVAL_task_0&&conv_cnt<=35)begin
        //task 0 step 1 img_0 conv ker_ch1_1
        ...
        //task 0 step 1 img_1 conv ker_ch1_2
        ...
        //task 1 step 1 img_0 conv ker_A
        ch1_mult_in_a[0]=img_0[conv_img_idx]; 
        ch1_mult_in_a[1]=img_0[conv_img_idx+1];
        ch1_mult_in_a[2]=img_0[conv_img_idx+2]; 
        ch1_mult_in_a[3]=img_0[conv_img_idx+8];
        ...
        ch1_mult_in_a[7]=img_0[conv_img_idx+17]; 
        ch1_mult_in_a[8]=img_0[conv_img_idx+18];
        //task 1 step 1 img_0 conv ker_B
        ch2_mult_in_a[0]=img_0[conv_img_idx]; 
        ch2_mult_in_a[1]=img_0[conv_img_idx+1];
        ch2_mult_in_a[2]=img_0[conv_img_idx+2]; 
        ch2_mult_in_a[3]=img_0[conv_img_idx+8];
        ch2_mult_in_a[4]=img_0[conv_img_idx+9]; 
        ch2_mult_in_a[5]=img_0[conv_img_idx+10];
        ch2_mult_in_a[6]=img_0[conv_img_idx+16];
        ch2_mult_in_a[7]=img_0[conv_img_idx+17]; 
        ch2_mult_in_a[8]=img_0[conv_img_idx+18];
    end
    else if(cur_state==EVAL_task_0&&conv_cnt<=71)begin  
        //task 0 step 1 img_0 conv ker_ch2_1
        ch1_mult_in_a[0]=img_0[conv_img_idx]; 
        ch1_mult_in_a[1]=img_0[conv_img_idx+1];
        ch1_mult_in_a[2]=img_0[conv_img_idx+2]; 
        ch1_mult_in_a[3]=img_0[conv_img_idx+8];
        ...
        ch1_mult_in_a[7]=img_0[conv_img_idx+17]; 
        ch1_mult_in_a[8]=img_0[conv_img_idx+18];
        //task 0 step 1 img_1 conv ker_ch2_2
        ch2_mult_in_a[0]=img_1[conv_img_idx]; 
        ch2_mult_in_a[1]=img_1[conv_img_idx+1];
        ch2_mult_in_a[2]=img_1[conv_img_idx+2]; 
        ch2_mult_in_a[3]=img_1[conv_img_idx+8];
        ...
        //task 1 step 1 img_0 conv ker_C
        ch1_mult_in_a[0]=img_0[conv_img_idx]; 
        ch1_mult_in_a[1]=img_0[conv_img_idx+1];
        ch1_mult_in_a[2]=img_0[conv_img_idx+2]; 
        ch1_mult_in_a[3]=img_0[conv_img_idx+8];
        ...
        ch1_mult_in_a[7]=img_0[conv_img_idx+17]; 
        ch1_mult_in_a[8]=img_0[conv_img_idx+18];
        //task 1 step 1 img_0 conv ker_D
        ch2_mult_in_a[0]=img_0[conv_img_idx]; 
        ch2_mult_in_a[1]=img_0[conv_img_idx+1];
        ch2_mult_in_a[2]=img_0[conv_img_idx+2]; 
        ch2_mult_in_a[3]=img_0[conv_img_idx+8];
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt==99)begin
        //task 0 step 4 FC 1 wght 0
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt==100)begin
        //task 0 step 4 FC 1 wght 2
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt==101)begin
        //task 0 step 4 FC 1 wght 4
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt==102)begin
        //task 0 step 5 relu
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt==103)begin
        //task 0 step 6 FC wght 0
        ...
        ch1_mult_in_a[5]=bias_2;        
        ch1_mult_in_a[6]=IEEE_zero;
        ch1_mult_in_a[7]=IEEE_zero;
        ch1_mult_in_a[8]=IEEE_zero;
        //task 0 step 6 FC wght 1
        ...
        ch2_mult_in_a[5]=bias_2;        
        ch2_mult_in_a[6]=IEEE_zero;
        ch2_mult_in_a[7]=IEEE_zero;
        ch2_mult_in_a[8]=IEEE_zero;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==104)begin
        //task 0 step 6 FC wght 2
...
always@(*)begin
    if(cur_state==EVAL_task_0&&conv_cnt<=35)begin
        ...
        ch1_mult_in_b[7]=ker_ch1[7][0]; 
        ch1_mult_in_b[8]=ker_ch1[8][0]; 
        //task 0 step 1 img_1 conv ker_B
        ch2_mult_in_b[0]=ker_ch1[0][1]; 
        ch2_mult_in_b[1]=ker_ch1[1][1]; 
        ch2_mult_in_b[2]=ker_ch1[2][1]; 
        ch2_mult_in_b[3]=ker_ch1[3][1]; 
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt>=36&&conv_cnt<=71)begin
        //task 0 step 1 img_0 conv ker_ch2_1
    ...
    else if(cur_state==EVAL_task_1&&conv_cnt>=36&&conv_cnt<=71)begin
        //task 1 step 1 img_0 conv ker_C
        ch1_mult_in_b[0]=ker_ch2[0][0]; 
        ch1_mult_in_b[1]=ker_ch2[1][0]; 
        ch1_mult_in_b[2]=ker_ch2[2][0]; 
        ...
        ch1_mult_in_b[7]=ker_ch2[7][0]; 
        ch1_mult_in_b[8]=ker_ch2[8][0]; 
        //task 1 step 1 img_1 conv ker_D
        ch2_mult_in_b[0]=ker_ch2[0][1]; 
        ch2_mult_in_b[1]=ker_ch2[1][1]; 
        ch2_mult_in_b[2]=ker_ch2[2][1]; 
        ch2_mult_in_b[3]=ker_ch2[3][1]; 
        ...
        //task 0 step 1 step 4 - FC wght 0
        ...
        //task 0 step 1 step 4 - FC wght 1
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        temp_cmp_result1<=IEEE_zero;  temp_cmp_result5<=IEEE_zero;
        temp_cmp_result2<=IEEE_zero;  temp_cmp_result6<=IEEE_zero;
        temp_cmp_result3<=IEEE_zero;  temp_cmp_result7<=IEEE_zero;
        temp_cmp_result4<=IEEE_zero;  temp_cmp_result8<=IEEE_zero;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==8'd72)begin
        temp_cmp_result1<=conved_img[0][0];  temp_cmp_result5<=conved_img[0][1];
        temp_cmp_result2<=conved_img[3][0];  temp_cmp_result6<=conved_img[3][1];
        temp_cmp_result3<=conved_img[18][0]; temp_cmp_result7<=conved_img[18][1];
        temp_cmp_result4<=conved_img[21][0]; temp_cmp_result8<=conved_img[21][1];
    end
    else if(cur_state==EVAL_task_0&&(conv_cnt>=8'd73&&conv_cnt<=8'd79))begin
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt==8'd80)begin
...
//Part 2 Comb Logic
always@(*)begin
    if(cur_state==EVAL_task_0&&conv_cnt==8'd73)begin
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt==8'd74)begin
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt==8'd75)begin
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt==8'd76)begin
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt==8'd77)begin
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt==8'd78)begin
    ...
    else if(cur_state==EVAL_task_0&&conv_cnt==8'd79)begin
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        div_num<=IEEE_zero; div_den<=IEEE_zero;  
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==83)begin
        if(mode_q==2'b01||mode_q==2'b11)div_num<=pooled_img[0];
        else div_num<=sub_out;
        div_den<=add_conv_result;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==85)begin
        if(mode_q==2'b01||mode_q==2'b11)div_num<=pooled_img[1];
        else div_num<=sub_out;
        div_den<=add_conv_result;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==87)begin
        if(mode_q==2'b01||mode_q==2'b11)div_num<=pooled_img[2];
        else div_num<=sub_out;
        div_den<=add_conv_result;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==89)begin
        if(mode_q==2'b01||mode_q==2'b11)div_num<=pooled_img[3];
        else div_num<=sub_out;
        div_den<=add_conv_result;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==91)begin
        if(mode_q==2'b01||mode_q==2'b11)div_num<=pooled_img[4];
        else div_num<=sub_out;
        div_den<=add_conv_result;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==93)begin
        if(mode_q==2'b01||mode_q==2'b11)div_num<=pooled_img[5];
        else div_num<=sub_out;
        div_den<=add_conv_result;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==95)begin
        if(mode_q==2'b01||mode_q==2'b11)div_num<=pooled_img[6];
        else div_num<=sub_out;
        div_den<=add_conv_result;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==97)begin
...
//Part 2 Comb Logic
always@(*)begin
    if(cur_state==EVAL_task_0&&(conv_cnt>=81&&conv_cnt<=98))begin
        div_in_a=div_num; div_in_b=div_den;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==109)begin
        div_in_a=exp_of_z[0]; div_in_b=sum_of_exp;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==110)begin
        div_in_a=exp_of_z[1]; div_in_b=sum_of_exp;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==111)begin
        div_in_a=exp_of_z[2]; div_in_b=sum_of_exp;
    end
    else begin
        div_in_a=exp_of_z[2]; div_in_b=sum_of_exp;
    end
...
//Part 2 Comb Logic 
always @(*) begin
    if(cur_state==EVAL_task_0&&conv_cnt==81) exp_in=pooled_img[0];   //e^x1
    else if(cur_state==EVAL_task_0&&conv_cnt==82) exp_in={~pooled_img[0][31],pooled_img[0][30:0]};   //e^-x1
    else if(cur_state==EVAL_task_0&&conv_cnt==83) exp_in=pooled_img[1];   //e^x2
    else if(cur_state==EVAL_task_0&&conv_cnt==84) exp_in={~pooled_img[1][31],pooled_img[1][30:0]};   //e^-x2
    else if(cur_state==EVAL_task_0&&conv_cnt==85) exp_in=pooled_img[2];   //e^x3
    else if(cur_state==EVAL_task_0&&conv_cnt==86) exp_in={~pooled_img[2][31],pooled_img[2][30:0]};   //e^-x3
    else if(cur_state==EVAL_task_0&&conv_cnt==87) exp_in=pooled_img[3];   //e^x4
    else if(cur_state==EVAL_task_0&&conv_cnt==88) exp_in={~pooled_img[3][31],pooled_img[3][30:0]};   //e^-x4
    else if(cur_state==EVAL_task_0&&conv_cnt==89) exp_in=pooled_img[4];   //e^x5
    else if(cur_state==EVAL_task_0&&conv_cnt==90) exp_in={~pooled_img[4][31],pooled_img[4][30:0]};   //e^-x5
    else if(cur_state==EVAL_task_0&&conv_cnt==91) exp_in=pooled_img[5];   //e^x6
    else if(cur_state==EVAL_task_0&&conv_cnt==92) exp_in={~pooled_img[5][31],pooled_img[5][30:0]};   //e^-x6
    else if(cur_state==EVAL_task_0&&conv_cnt==93) exp_in=pooled_img[6];   //e^x7
    else if(cur_state==EVAL_task_0&&conv_cnt==94) exp_in={~pooled_img[6][31],pooled_img[6][30:0]};   //e^-x7
    else if(cur_state==EVAL_task_0&&conv_cnt==95) exp_in=pooled_img[7];   //e^x8
    else if(cur_state==EVAL_task_0&&conv_cnt==96) exp_in={~pooled_img[7][31],pooled_img[7][30:0]};   //e^-x8
    //step 7 sofmax
    else if(cur_state==EVAL_task_0&&conv_cnt==105) exp_in=img_after_FC2[0];
    else if(cur_state==EVAL_task_0&&conv_cnt==106) exp_in=img_after_FC2[1];
    else if(cur_state==EVAL_task_0&&conv_cnt==107) exp_in=img_after_FC2[2];
    else exp_in=img_after_FC2[2];
end
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        cur_positive_exp<=IEEE_zero; cur_negative_exp<=IEEE_zero;
        prev_positive_exp<=IEEE_zero; prev_negative_exp<=IEEE_zero;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==81)cur_positive_exp<=exp_out;  //e^x1
    else if(cur_state==EVAL_task_0&&conv_cnt==82)begin
        cur_negative_exp<=exp_out;  //e^-x1
        prev_positive_exp<=cur_positive_exp;//e^x1
        prev_negative_exp<=exp_out;         //e^-x1
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==83)cur_positive_exp<=exp_out;  //e^x2
    else if(cur_state==EVAL_task_0&&conv_cnt==84)begin
        cur_negative_exp<=exp_out;  //e^-x2
        prev_positive_exp<=cur_positive_exp;//e^x2
        prev_negative_exp<=exp_out;         //e^-x2
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==85)cur_positive_exp<=exp_out;  //e^x3
    else if(cur_state==EVAL_task_0&&conv_cnt==86)begin
        cur_negative_exp<=exp_out;  //e^-x3
        prev_positive_exp<=cur_positive_exp;//e^x3
        prev_negative_exp<=exp_out;         //e^-x3
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==87)cur_positive_exp<=exp_out;  //e^x4
    else if(cur_state==EVAL_task_0&&conv_cnt==88)begin
        cur_negative_exp<=exp_out;  //e^-x4
        prev_positive_exp<=cur_positive_exp;//e^x4
        prev_negative_exp<=exp_out;         //e^-x4
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==89)cur_positive_exp<=exp_out;  //e^x5
    else if(cur_state==EVAL_task_0&&conv_cnt==90)begin
        cur_negative_exp<=exp_out;  //e^-x5
        prev_positive_exp<=cur_positive_exp;//e^x5
        prev_negative_exp<=exp_out;         //e^-x5
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==91)cur_positive_exp<=exp_out;  //e^x6
    else if(cur_state==EVAL_task_0&&conv_cnt==92)begin
        cur_negative_exp<=exp_out;  //e^-x6
        prev_positive_exp<=cur_positive_exp;//e^x6
        prev_negative_exp<=exp_out;         //e^-x6
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==93)cur_positive_exp<=exp_out;  //e^x7
    else if(cur_state==EVAL_task_0&&conv_cnt==94)begin
        cur_negative_exp<=exp_out;  //e^-x7
        prev_positive_exp<=cur_positive_exp;//e^x7
        prev_negative_exp<=exp_out;         //e^-x7
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==95)cur_positive_exp<=exp_out;  //e^x7
    else if(cur_state==EVAL_task_0&&conv_cnt==96)begin
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)begin
        for(i=0;i<8;i=i+1)acted_img[i]<=IEEE_zero;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==84) acted_img[0]<=div_out;
    else if(cur_state==EVAL_task_0&&conv_cnt==86) acted_img[1]<=div_out;
    else if(cur_state==EVAL_task_0&&conv_cnt==88) acted_img[2]<=div_out;
    else if(cur_state==EVAL_task_0&&conv_cnt==90) acted_img[3]<=div_out;
    else if(cur_state==EVAL_task_0&&conv_cnt==92) acted_img[4]<=div_out;
    else if(cur_state==EVAL_task_0&&conv_cnt==94) acted_img[5]<=div_out;
    else if(cur_state==EVAL_task_0&&conv_cnt==96) acted_img[6]<=div_out;
    else if(cur_state==EVAL_task_0&&conv_cnt==98) acted_img[7]<=div_out;
    else if(cur_state==CLEAR)begin
        for(i=0;i<8;i=i+1)acted_img[i]<=IEEE_zero;
    end
end
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        for(i=0;i<5;i=i+1)img_after_FC1[i]<=IEEE_zero;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==99)begin
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        for(i=0;i<5;i=i+1)img_after_relu[i]<=IEEE_zero;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==102)begin
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        for(i=0;i<3;i=i+1)img_after_FC2[i]<=IEEE_zero;
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==103)begin
...
//===STEP 7 softmax===
reg [31:0] img_after_softmax [0:2];
//reg [31:0] exp_of_z[0:2];   
//reg [31:0] sum_of_exp;      

always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        sum_of_exp<=IEEE_zero;
        for(i=0;i<3;i=i+1)begin
            exp_of_z[i]<=IEEE_zero; img_after_softmax[i]<=IEEE_zero;
        end
    end
    else if(cur_state==EVAL_task_0&&conv_cnt==105)exp_of_z[0]<=exp_out;
    else if(cur_state==EVAL_task_0&&conv_cnt==106)exp_of_z[1]<=exp_out;
    else if(cur_state==EVAL_task_0&&conv_cnt==107)exp_of_z[2]<=exp_out;
    else if(cur_state==EVAL_task_0&&conv_cnt==108)sum_of_exp<=sum3_out[0];
    else if(cur_state==EVAL_task_0&&conv_cnt==109)img_after_softmax[0]<=div_out;
    else if(cur_state==EVAL_task_0&&conv_cnt==110)img_after_softmax[1]<=div_out;
    else if(cur_state==EVAL_task_0&&conv_cnt==111)img_after_softmax[2]<=div_out;
    else if(cur_state==CLEAR)begin
        sum_of_exp<=IEEE_zero;
        for(i=0;i<3;i=i+1)begin
            exp_of_z[i]<=IEEE_zero; img_after_softmax[i]<=IEEE_zero;
...
//===Task 1 step 3===
//reg [31:0] task1_step3_in_1,task1_step3_in_2,task1_step3_in_3,task1_step3_in_4;
reg [5:0] capacity_sum;
reg [31:0] result_sum;

//Part 2 Comb Logic
reg [31:0] cmp_task1_step3_a,cmp_task1_step3_b; wire cmp_task1_step3_altb;
DW_fp_cmp_inst cmp_task1_step3(.inst_a(cmp_task1_step3_a), .inst_b(cmp_task1_step3_b),
.inst_zctr(1'b0),.aeqb_inst(),.altb_inst(cmp_task1_step3_altb),.agtb_inst(),
.unordered_inst(),.z0_inst(),.z1_inst(),.status0_inst(),.status1_inst());
//=================

reg [31:0] cur_max_result;  
always@(*)begin
    if(cur_state==EVAL_task_1&&conv_cnt>=84&&conv_cnt<=99)begin
        cmp_task1_step3_a=cur_max_result;
        cmp_task1_step3_b=result_sum;
    end
    else begin
        cmp_task1_step3_a=cur_max_result;
        cmp_task1_step3_b=result_sum;
...
always@(*)begin
    if(cur_state==EVAL_task_1&&conv_cnt==84)begin
        //c(4,4)
        task1_step3_in_1=A_result; task1_step3_in_2=B_result;
        task1_step3_in_3=C_result; task1_step3_in_4=D_result;
        capacity_sum=capacity_cost_rarr[0]+capacity_cost_rarr[1]+capacity_cost_rarr[2]+capacity_cost_rarr[3];
        result_sum=sum3_out[3];
    end

    else if(cur_state==EVAL_task_1&&conv_cnt==85)begin
        //c(4,3) B C D
        task1_step3_in_1=IEEE_zero;task1_step3_in_2=B_result;
        task1_step3_in_3=C_result; task1_step3_in_4=D_result;
        capacity_sum=capacity_cost_rarr[1]+capacity_cost_rarr[2]+capacity_cost_rarr[3];
        result_sum=sum3_out[3];
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==86)begin
        //c(4,3) A C D
        task1_step3_in_1=A_result; task1_step3_in_2=IEEE_zero;
        task1_step3_in_3=C_result; task1_step3_in_4=D_result;
        capacity_sum=capacity_cost_rarr[0]+capacity_cost_rarr[2]+capacity_cost_rarr[3];
        result_sum=sum3_out[3];
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==87)begin
        //c(4,3) A B D
        task1_step3_in_1=A_result; task1_step3_in_2=B_result;
        task1_step3_in_3=IEEE_zero;task1_step3_in_4=D_result;
        capacity_sum=capacity_cost_rarr[0]+capacity_cost_rarr[1]+capacity_cost_rarr[3];
        result_sum=sum3_out[3];
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==88)begin
        //c(4,3) A B C 
        task1_step3_in_1=A_result; task1_step3_in_2=B_result;
        task1_step3_in_3=C_result; task1_step3_in_4=IEEE_zero;
        capacity_sum=capacity_cost_rarr[0]+capacity_cost_rarr[1]+capacity_cost_rarr[2];
        result_sum=sum3_out[3];
    end

    else if(cur_state==EVAL_task_1&&conv_cnt==89)begin
        //c(4,2) A B  
        task1_step3_in_1=A_result; task1_step3_in_2=B_result;
        task1_step3_in_3=IEEE_zero;task1_step3_in_4=IEEE_zero;
        capacity_sum=capacity_cost_rarr[0]+capacity_cost_rarr[1];
        result_sum=sum3_out[3];
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==90)begin
        //c(4,2) A C  
        task1_step3_in_1=A_result; task1_step3_in_2=IEEE_zero;
        task1_step3_in_3=C_result; task1_step3_in_4=IEEE_zero;
        capacity_sum=capacity_cost_rarr[0]+capacity_cost_rarr[2];
        result_sum=sum3_out[3];
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==91)begin
        //c(4,2) A D  
        task1_step3_in_1=A_result; task1_step3_in_2=IEEE_zero;
        task1_step3_in_3=IEEE_zero;task1_step3_in_4=D_result;
        capacity_sum=capacity_cost_rarr[0]+capacity_cost_rarr[3];
        result_sum=sum3_out[3];
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==92)begin
        //c(4,2) B C 
        task1_step3_in_1=IEEE_zero;task1_step3_in_2=B_result;
        task1_step3_in_3=C_result; task1_step3_in_4=IEEE_zero;
        capacity_sum=capacity_cost_rarr[1]+capacity_cost_rarr[2];
        result_sum=sum3_out[3];
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==93)begin
        //c(4,2) B D
        task1_step3_in_1=IEEE_zero;task1_step3_in_2=B_result;
        task1_step3_in_3=IEEE_zero;task1_step3_in_4=D_result;
        capacity_sum=capacity_cost_rarr[1]+capacity_cost_rarr[3];
        result_sum=sum3_out[3];
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==94)begin
        //c(4,2) C D
        task1_step3_in_1=IEEE_zero;task1_step3_in_2=IEEE_zero;
        task1_step3_in_3=C_result; task1_step3_in_4=D_result;
        capacity_sum=capacity_cost_rarr[2]+capacity_cost_rarr[3];
        result_sum=sum3_out[3];
    end

    else if(cur_state==EVAL_task_1&&conv_cnt==95)begin
        //c(4,1) A
        task1_step3_in_1=A_result;  task1_step3_in_2=IEEE_zero;
        task1_step3_in_3=IEEE_zero; task1_step3_in_4=IEEE_zero;
        capacity_sum=capacity_cost_rarr[0];
        result_sum=sum3_out[3];
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==96)begin
        //c(4,1) B
        task1_step3_in_1=IEEE_zero; task1_step3_in_2=B_result;
        task1_step3_in_3=IEEE_zero; task1_step3_in_4=IEEE_zero;
        capacity_sum=capacity_cost_rarr[1];
        result_sum=sum3_out[3];
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==97)begin
        //c(4,1) C
        task1_step3_in_1=IEEE_zero; task1_step3_in_2=IEEE_zero;
        task1_step3_in_3=C_result;  task1_step3_in_4=IEEE_zero;
        capacity_sum=capacity_cost_rarr[2];
        result_sum=sum3_out[3];
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==98)begin
        //c(4,1) D
        task1_step3_in_1=IEEE_zero; task1_step3_in_2=IEEE_zero;
        task1_step3_in_3=IEEE_zero; task1_step3_in_4=D_result;
        capacity_sum=capacity_cost_rarr[3];
        result_sum=sum3_out[3];
    end

    else if(cur_state==EVAL_task_1&&conv_cnt==99)begin
        //c(4,0) NULL
        task1_step3_in_1=IEEE_zero; task1_step3_in_2=IEEE_zero;
        task1_step3_in_3=IEEE_zero; task1_step3_in_4=IEEE_zero;
        capacity_sum=6'd0;
...
//reg [31:0] cur_max_result;  //a
reg [3:0] cur_choice;
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        cur_max_result<=IEEE_zero; cur_choice<=4'b0000;
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==84)begin
        //c(4,4)
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b1111;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
    end
    else if(cur_state==EVAL_task_1&&conv_cnt==85)begin
        //c(4,3) B C D
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b0111;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,3) A C D
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b1011;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,3) A B D
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b1101;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,3) A B C 
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b1110;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,2) A B  
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b1100;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,2) A C   
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b1010;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,2) A D  
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b1001;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,2) B C  
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b0110;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,2) B D  
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b0101;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,2) C D
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b0011;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,1) A
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b1000;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,1) B
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b0100;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,1) C
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b0010;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,1) D
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b0001;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
        ...
        //c(4,0) NULL
        if(cmp_task1_step3_altb==1'b1&&capacity_sum<=capacity_q)begin
            cur_max_result<=result_sum; cur_choice<=4'b0000;
        end
        else begin
            cur_max_result<=cur_max_result; cur_choice<=cur_choice;
        end
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        out_cnt<=2'd0; out_valid<=1'b0; out<=32'd0;
    end
    else if(cur_state==OUTPUT&&out_cnt<=2'd2&&task_number_q==0)begin
        out<=img_after_softmax[out_cnt];
        out_cnt<=out_cnt+1'd1; out_valid<=1'b1;
    end
    else if(cur_state==OUTPUT&&out_cnt<=2'd0&&task_number_q==1)begin
        out<={28'd0,cur_choice};
        out_cnt<=out_cnt+1'd1; out_valid<=1'b1;
    end
    else begin
        out_cnt<=2'd0; out_valid<=1'b0; out<=32'd0;
