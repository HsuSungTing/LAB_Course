...
parameter IDLE=3'd0,EVAL_check=3'd1, EVAL_place=3'd2, OUTPUT=3'd3, CLEAR=4'd4;
reg EVAL_check_done, EVAL_place_done, OUTPUT_done;
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
    ...
    else if(cur_state==EVAL_check&&pt_cnt<=2)begin
    ...
    else if(cur_state==EVAL_check&&pt_cnt==3)begin
    ...
    else if(cur_state==EVAL_check && pt_cnt>3 && hull_cur_idx<=hull_len-1'd1)begin
        ...
        if(lt_not_coline_bool)lt_coline_idx<=hull_cur_idx;
        ...
        if(rt_not_coline_bool)rt_coline_idx<=hull_cur_idx;
        ...
        if(on_inner_seg_bool==1'b1||outside_tag==1'b0)begin
            //直接drop輸入的pt
            go_output_tag<=1'b1; drop_len<=9'd1;
            drop_x[0]<=in_x_q; drop_y[0]<=in_y_q;
        end
        place_ptr<=lt_prim;             //為了EVAL_place
        drop_ptr<=(rt_prim+1)%hull_len; //為了EVAL_place
    end
    else if(cur_state==EVAL_place&&(hull_placed_done==1'b0||drop_placed_done==1'b0))begin
        //case 4 Place
        //step 1 放入updated_hull(從lt`放到rt`),且hull_len要更新
        if(place_ptr!=(rt_prim+1) % hull_len)begin
