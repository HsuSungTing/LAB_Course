...
//Part 3 Seq Logic
reg [7:0] old_hull_len;
reg [9:0] old_hull_x [0:127];
reg [9:0] old_hull_y [0:127];
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        for(i=0;i<128;i=i+1)begin
            ...
            old_hull_x[i]<=10'd0; old_hull_y[i]<=10'd0;
            updated_hull_x[i]<=10'd0; updated_hull_y[i]<=10'd0;
        end
        hull_cur_idx<=8'd0; old_hull_len<=8'd0;
        ...
        old_hull_len<=hull_len;
        for(i=0;i<128;i=i+1)begin
            old_hull_x[i]<=hull_x[i]; old_hull_y[i]<=hull_y[i];
        end
        ...
        if(place_ptr!=rt_prim_plus_one||pass_rt_prim_bool==1'b0)begin
            ...
            place_ptr<=(place_ptr+1) % old_hull_len;
        ...
        if((drop_ptr!=lt_prim||pass_lt_prim_minus_one_bool==1'b0)&&rt_prim_plus_one!=lt_prim)begin
            drop_x[drop_len]<=old_hull_x[drop_ptr];
            drop_y[drop_len]<=old_hull_y[drop_ptr];
            drop_len<=drop_len+1'd1;
            drop_ptr<=(drop_ptr+1'd1)%old_hull_len;
