...
reg on_inner_seg_bool,inside_bool;
...
task update_hull_drop_outxy;begin
	if(pt_cnt<=2)begin
		...
		inside_bool=1; on_inner_seg_bool=0;
		...
		for(i=0;i<hull_len;i=i+1)begin
			...
			if(cross_check_inside==2'd0)inside_bool=0;
			...
			//cross(p, cur, next)
			cross_check(in_x_d,in_y_,hull_x[hull_cur_idx],hull_y[hull_cur_idx],
			...
			if(check_p_cur_prev==2'd1 && check_p_cur_next==2'd2)begin 
