...
always@(*)begin
    ...
    OUTPUT: begin
        if(pt_cnt<=pt_num_q-1'd1&&out_cnt>=out_cnt_bound-1) next_state=CLEAR_small;
        else if(pt_cnt==pt_num_q-1'd1&&out_cnt>=out_cnt_bound-1)next_state=CLEAR_big;
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        ...
        if(place_ptr!=rt_prim_plus_one)begin
        ...
        if(drop_ptr!=lt_prim)begin
            ...
            drop_placed_done<=1'b1; out_cnt_bound<=drop_len;
        end
    end
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        ...
        if(drop_len==8'd0)begin 
            ...
            out_x<=drop_x[out_cnt]; out_y<=drop_x[out_cnt];
        end
        drop_num<=drop_len;
    end
    else begin
