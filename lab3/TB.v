...
task update_hull_drop_outxy;begin
	if(pt_cnt<=2)begin
		...
		//=====
		//===for debug===
		$display("in_x_d: %d, in_y_d: %d",in_x_d,in_y_d);
		//===============
		for(i=0;i<hull_len;i=i+1)begin
			...
			//===for debug===
			$display("hull_cur_idx: %d",hull_cur_idx);
			$display("hull_prev_idx: %d",hull_prev_idx);
			$display("hull_next_idx: %d",hull_next_idx);
			//===============
		...
		//===for debug===
		$display("check_p_cur_next: %d",check_p_cur_next);
		$display("check_p_cur_prev: %d",check_p_cur_prev);
		$display("check_cur_p_next: %d",check_cur_p_next);
		$display("check_cur_p_prev: %d",check_cur_p_prev);
		//===============
...
task check_spec_9_task; begin
	...
	while(out_valid_cycle_cnt<drop_num_q||out_valid_cycle_cnt==0)begin
		if(out_valid===1)begin
			out_valid_cycle_cnt=out_valid_cycle_cnt+1;
		end
		@(negedge clk);
